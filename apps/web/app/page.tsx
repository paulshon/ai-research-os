...
const AROS_TIERS = [
  {
    key: "free", name: "Free", desc: "개인 연구자를 위한 무료 플랜", popular: false,
    feats: ["Dashboard", "연구설계", "문헌연구", "논문작성 (양적·질적)"],
    cta: "무료로 시작", href: "/signup?plan=free",
  },
  {
    key: "basic", name: "Basic", desc: "꾸준한 연구자를 위한 플랜", popular: false,
    feats: ["Dashboard", "연구설계", "문헌연구", "논문작성 (양적·질적·혼합·실험연구)", "검토·검증", "논문일정", "논문분석", "논문크리틱"],
    cta: "Basic 시작하기", href: "/signup?plan=basic",
  },
  {
    key: "scholar", name: "Scholar", desc: "본격적인 논문 작성을 위한 플랜", popular: true,
    feats: ["Dashboard", "연구설계", "문헌연구", "논문작성 (양적·질적·혼합·실험·체계적문헌고찰·메타분석·서술적·범위 문헌연구)", "검토·검증", "논문일정", "논문분석", "논문크리틱", "문장라이브러리", "참고문헌정리"],
    cta: "Scholar 시작하기", href: "/signup?plan=scholar",
  },
  {
    key: "university", name: "University", desc: "대학·연구기관 팀 플랜", popular: false,
    feats: ["논문작성 (전체 연구유형 지원)", "연구방법 엔진", "검토·검증 · 논문일정 · 논문분석", "문장라이브러리 · 참고문헌정리", "양적/질적 연구방법 교육", "연구설계 교육", "기관 라이선스 · 연구실 협업", "교수 전용 관리도구"],
    cta: "문의하기", href: "/signup?plan=university",
  },
] as const;
...
export default function LandingPage() {
  ...
  const NAV_LINKS = [
    { href: "/rdos", label: "연구준비자 플랜" },
    { href: "/dashboard", label: "연구자 플랜" },
    ...
    <div data-page="landing" className="min-h-screen bg-[#070708] text-[#f4f4f5] overflow-x-hidden font-nanum-gothic antialiased">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? "bg-[#070708]/85 backdrop-blur-xl border-b border-white/[0.07]" : "bg-transparent"}`}>
        <div className="max-w-[1280px] mx-auto px-6 h-[76px] flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="text-[17px] font-bold tracking-tight font-nanum-gothic">AI Research <span className="text-[#ff7a00]">OS</span></span>
          </Link>
      ...
      <section className="relative pt-[84px] md:pt-[150px] pb-24 px-6 overflow-hidden">
        <div className="max-w-[1080px] mx-auto text-center relative z-10 pt-2 md:pt-10">
          ...
          >
            {t("landing.heroDescLine1")}
            <br />
            <span className="text-white/45">{t("landing.heroDescLine2")}</span>
          </motion.h1>
          <motion.p
      ...
      <section id="tracks" className="py-24 px-6 border-t border-white/[0.08]">
        <div className="max-w-[1080px] mx-auto">
          <div className="text-center mb-12">
            <p className="text-[12px] uppercase tracking-[0.2em] text-white/35 mb-4 font-mono">choose your track</p>
            ...
            <p className="text-white/50 max-w-xl mx-auto leading-relaxed text-[15px] mt-4">
              회원가입 후 트랙을 선택하면 각 플랫폼으로 분기됩니다.
            </p>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            {[
              { href: "/rdos", accent: "#3ecfb2", title: "연구준비자 플랜", sub: "RDOS · Researcher Development OS",
                audience: "대학원 진학 예정자 · 석사과정생 · 연구 입문자", purpose: "논문 및 연구 역량 개발 (교육 중심)", cta: "RDOS 입장" },
              { href: "/dashboard", accent: "#ff7a00", title: "연구자 플랜", sub: "AI-Research-OS",
                audience: "석사 이상 연구 경험자 · 논문 저자 · 교수 · 연구원", purpose: "실제 연구 수행 및 논문 생산", cta: "AI-Research-OS 입장" },
            ].map((tr) => (
              <div key={tr.href} className="flex flex-col p-8 rounded-[20px] bg-[#0c0c0e] border border-white/[0.08] hover:border-white/[0.18] transition-all duration-300">
                ...
                <p className="text-[13.5px] text-white/55 leading-[1.7] mb-1"><span className="text-white/40">대상</span> · {tr.audience}</p>
                <p className="text-[13.5px] text-white/55 leading-[1.7] mb-7"><span className="text-white/40">목적</span> · {tr.purpose}</p>
          ...
          <div className="text-center mt-8">
            <Link href="/start" className="text-[13px] text-white/45 hover:text-white/80 transition">플랜 선택 페이지에서 자세히 보기 →</Link>
          </div>
        </div>
      </section>
      ...
      <section id="pricing" className="py-32 px-6 bg-[#0c0c0e] border-y border-white/[0.07]">
        <div className="max-w-[1180px] mx-auto">
          ...
          >
            ...
            <h2 className="font-nanum-bold tracking-[-0.01em] leading-[1.15] text-[clamp(1.5rem,3.4vw,2.4rem)]">연구에 맞는 플랜을 선택하세요</h2>
            <p className="text-white/45 mt-4 text-[14px]">연구자 플랜(AI-Research-OS)과 연구준비자 플랜(RDOS), 두 트랙으로 운영됩니다.</p>
          </motion.div>

          {/* ── 연구자 플랜 (AI-Research-OS) : Free / Basic / Scholar / University ── */}
          <div className="flex items-center gap-3 mb-6">
            <span className="w-2 h-2 rounded-full bg-[#ff7a00]" />
            <h3 className="text-[17px] font-bold">연구자 플랜 · AI-Research-OS</h3>
            <span className="text-[12px] text-white/40">석사 이상 연구 경험자 · 논문 저자 · 교수 · 연구원</span>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-16">
            {AROS_TIERS.map((tier) => (
              <div key={tier.key} className={`relative flex flex-col p-7 rounded-[20px] border transition-all duration-300 ${tier.popular ? "bg-white border-white text-[#070708] shadow-2xl lg:scale-[1.03]" : "bg-[#070708] border-white/[0.08] hover:border-white/[0.16]"}`}>
                ...
                <p className={`text-[12.5px] mb-5 ${tier.popular ? "text-[#070708]/60" : "text-white/50"}`}>{tier.desc}</p>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-[17px] font-bold">연구준비자 플랜 · RDOS</h3>
            <span className="text-[12px] text-white/40">대학원 진학 예정자 · 석사과정생 · 연구 입문자</span>
          </div>
          <div className="rounded-[22px] border border-[#3ecfb2]/20 bg-[#070708] p-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-7">
              <div>
                <p className="text-[13px] text-white/55 leading-relaxed max-w-[560px]">
                  논문·연구 역량을 단계적으로 기르는 교육 트랙. 초보자를 Research-Ready Scholar(L9)로 성장시킵니다.
                </p>
              </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
              {RDOS_OFFERINGS.map(([menu, provides]) => (
                <div key={menu} className="p-4 rounded-[13px] bg-[#0c0c0e] border border-white/[0.07]">
                  <p className="text-[13px] font-semibold text-white/85 leading-tight mb-1">{menu}</p>
                  <p className="text-[11.5px] text-white/45">{provides}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-10 mb-12">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-[15px] font-bold">AI Research <span className="text-[#ff7a00]">OS</span></span>
              </div>
