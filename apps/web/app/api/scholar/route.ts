...
async function searchDOAJ(query: string, pageSize: number): Promise<PaperResult[]> {
  const url = `https://doaj.org/api/search/articles/${encodeURIComponent(query)}?page=1&pageSize=${Math.min(pageSize, 30)}`;
  try {
    const res = await fetch(url, { signal: AbortSignal.timeout(15000) });
    if (!res.ok) return [];
    const data = await res.json();
    return (data.results ?? []).map((row: any, i: number) => {
      ...
      return {
        id: `doaj_${i}_${row.id ?? i}`,
        title,
        authors: (bib.author ?? [])
          .map((a: any) => a.name ?? "")
          .filter(Boolean)
          .join(", ") || "Unknown",
        year: bib.year ? parseInt(String(bib.year), 10) : null,
        journal: bib.journal?.title ?? "",
        abstract: (bib.abstract ?? "").slice(0, 600),
        doi,
        url: doi ? `https://doi.org/${doi}` : bib.link?.[0]?.url ?? "https://doaj.org",
        citations: 0,
        keywords: (bib.keywords ?? []).slice(0, 5),
        source: "DOAJ",
        similarity: 0.82 - i * 0.006,
      };
    });
  } catch {
    return [];
...
async function searchArxiv(query: string, max: number): Promise<PaperResult[]> {
  const url = `https://export.arxiv.org/api/query?search_query=all:${encodeURIComponent(query)}&start=0&max_results=${Math.min(max, 30)}`;
  try {
    const res = await fetch(url, { signal: AbortSignal.timeout(15000) });
    if (!res.ok) return [];
    ...
    return entries.map((entry, i) => {
      const t = entry.match(/<title>([^<]*)<\/title>/)?.[1]?.replace(/\s+/g, " ").trim() ?? "Untitled";
...
async function searchPubMed(query: string, retmax: number): Promise<PaperResult[]> {
  try {
    const esearch = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodeURIComponent(query)}&retmax=${Math.min(retmax, 25)}&retmode=json`;
    const sRes = await fetch(esearch, { signal: AbortSignal.timeout(15000) });
    if (!sRes.ok) return [];
    ...
    const eRes = await fetch(esummary, { signal: AbortSignal.timeout(15000) });
    if (!eRes.ok) return [];
    const eData = await eRes.json();
    const result: PaperResult[] = [];
    ids.forEach((pmid, i) => {
      const item = eData.result?.[pmid];
      if (!item || item.error) return;
      const title = item.title ?? "Untitled";
      const year = parseInt(item.pubdate?.slice(0, 4) ?? "", 10) || null;
      const doi = item.elocationid?.replace(/^doi:\s*/i, "") ?? "";
      result.push({
        id: `pubmed_${pmid}`,
        title,
...
export async function GET(req: NextRequest) {
  ...
  const method: ScholarMethodBoost = ["quantitative", "qualitative", "mixed", "experimental", "all"].includes(
    methodParam,
  )
    ? methodParam
    : "all";
  const projectKeywords = (searchParams.get("projectKeywords") ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  try {
    ...
    if (region === "domestic") {
      ...
      // ── 해외 검색 ── (v41: 소스별 최대 추출량까지 끌어올려 통합)
      //   OpenAlex per_page ≤ 200, CrossRef rows ≤ 200(안전), Semantic Scholar limit ≤ 100
      const oaN = Math.min(limit, 200);
      const crN = Math.min(limit, 200);
      const s2N = Math.min(limit, 100);
      const [oa, cr, s2, doaj, arx, pub] = await Promise.allSettled([
        searchOpenAlex(query, oaN),
        searchCrossRef(query, crN),
        searchSemanticScholar(query, s2N),
        searchDOAJ(query, Math.min(25, limit)),
        searchArxiv(query, Math.min(20, limit)),
        searchPubMed(query, Math.min(20, limit)),
      ]);
      ...
      if (doaj.status === "fulfilled") allResults.push(...doaj.value);
      if (arx.status === "fulfilled") allResults.push(...arx.value);
      if (pub.status === "fulfilled") allResults.push(...pub.value);
