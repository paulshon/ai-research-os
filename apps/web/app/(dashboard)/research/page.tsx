...
export default function ResearchPage() {
  ...
  const getData = useCallback(
    () => ({ active, query, canvasOutput, copilotInput, copilotOutput }),
    [active, query, canvasOutput, copilotInput, copilotOutput]
  );
  const handleLoad = useCallback((data: unknown) => {
    const d = data as { active?: string; query?: string; canvasOutput?: string; copilotInput?: string; copilotOutput?: string };
  ...
  const handleReset = useCallback(() => {
    setActive("topic"); setQuery(""); setCanvasOutput(""); setCopilotInput(""); setCopilotOutput("");
  }, []);
  ...
  return (
    <div className="p-4 md:p-5 lg:px-6 lg:py-6 font-nanum-gothic">
      ...
      <div className="max-w-[1680px] mx-auto">
        <p className="text-[14px] text-white/20 font-mono mb-1">Research — AI Research OS</p>
        <h1 className="font-nanum-myeongjo text-[25px] font-bold text-[#e8eaf0] mb-1">연구 설계</h1>
        <p className="text-[16px] text-white/35 mb-6">주제 탐색부터 방법론 설계까지 — 연구의 기초를 잡으세요</p>
          ...
          {RESEARCH_SECTIONS.map((s) => (
            ...
            >
              <div className="flex items-center gap-2.5 mb-2">
                ...
                <span className="text-[17px] font-semibold text-[#e8eaf0]">{s.title}</span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
          <div className="p-6 rounded-[16px] bg-[#13161e] border border-white/[0.04] min-h-[400px]">
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-[16px] font-semibold text-[#e8eaf0]">{section.title} 캔버스</p>
            </div>
          ...
          <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04]">
            <p className="text-[16px] font-semibold text-[#6c8cff] mb-3 flex items-center gap-1.5"><Icon name="engine" size={15} />AI Research Copilot</p>
            ...
            />
            <button onClick={askCopilot} disabled={loading} className="w-full mt-2 py-2 rounded-lg bg-[#6c8cff]/20 text-[#6c8cff] text-[15px] disabled:opacity-40">{loading ? "..." : "AI 분석"}</button>
            ...
            <div className="mt-3 space-y-2">
              <Link href="/structure" className="block text-[14px] text-[#6c8cff] hover:underline">→ 구조 엔진으로 이동</Link>
            </div>
          </div>
        </div>
      </div>
