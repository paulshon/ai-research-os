    ...
    <div className="p-8 font-nanum-gothic">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className="text-[11px] text-white/25 font-['JetBrains_Mono',monospace] mb-1">Workspace</p>
            <h1 className="text-[24px] font-bold font-nanum-myeongjo">{t("workspace.title")}</h1>
          </div>
        ...
        {showCreate && (
          <div className="mb-6 p-6 rounded-[18px] bg-[#13161e] border border-white/[0.04]">
            <h3 className="text-[14px] font-semibold mb-4">{t("workspace.createTitle")}</h3>
            <div className="space-y-3">
              <input
                value={newTitle}
                onChange={(e) => setNewTitle(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && createProject()}
                className="w-full px-4 py-3 rounded-xl bg-[#1a1e2a] border border-white/[0.06] text-[14px] text-white focus:border-[#6c8cff] focus:outline-none"
                placeholder={t("workspace.titlePlaceholder")}
                autoFocus
              />
              <div>
                <label className="block text-[12px] text-white/30 mb-2">{t("workspace.researchType")}</label>
                <select
                  value={newType}
                  onChange={(e) => setNewType(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl bg-[#1a1e2a] border border-white/[0.06] text-[14px] text-white/60"
                >
                  {categories.map((cat) => (
              ...
              <div>
                <label className="block text-[12px] text-white/30 mb-2">로컬 저장 디렉토리 (선택사항)</label>
                <div className="flex gap-2">
                  <input
                    value={newLocalDir}
                    onChange={(e) => setNewLocalDir(e.target.value)}
                    className="flex-1 px-4 py-3 rounded-xl bg-[#1a1e2a] border border-white/[0.06] text-[14px] text-white/60 focus:border-[#6c8cff] focus:outline-none"
                    placeholder="~/Documents/Research"
                  />
                  <button onClick={handleSelectDir} className="px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-[13px] text-white/50 hover:text-white/70 whitespace-nowrap">
                    <Icon name="📁" className="inline-flex align-[-0.125em] mr-1" size={15} />선택
                  </button>
                </div>
                <p className="text-[11px] text-white/20 mt-1">프로젝트 파일이 저장될 로컬 경로를 지정합니다.</p>
              </div>
              <div className="flex gap-2">
                <button onClick={createProject} className="px-5 py-2.5 bg-[#4a6cf7] text-white rounded-xl text-[13px] font-medium">
                  {t("workspace.create")}
                </button>
                <button onClick={() => setShowCreate(false)} className="px-5 py-2.5 bg-white/[0.04] text-white/50 rounded-xl text-[13px]">
                  {t("common.cancel")}
                </button>
              </div>
            </div>
        ...
        {projects.length === 0 && !showCreate ? (
          <div className="p-12 rounded-[18px] bg-[#13161e] border border-white/[0.04] text-center">
            ...
            <p className="text-[15px] font-semibold mb-2">{t("workspace.emptyTitle")}</p>
            <p className="text-[13px] text-white/30 mb-5">{t("workspace.emptyDesc")}</p>
            <div className="flex gap-3 justify-center">
              <button onClick={() => setShowCreate(true)} className="px-6 py-3 bg-[#4a6cf7] text-white rounded-[10px] text-[13px] font-medium">
                {t("sidebar.newProject")}
              </button>
              <button onClick={handleLoadProject} className="px-6 py-3 bg-white/[0.04] text-white/60 border border-white/[0.06] rounded-[10px] text-[13px] font-medium hover:bg-white/[0.07]">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects.map((p) => (
              ...
              >
                <h3 className="text-[14px] font-semibold group-hover:text-[#6c8cff]">{p.title}</h3>
                <p className="text-[11px] text-white/25 mt-1">
                  {allTypes.find((ty) => ty.id === p.thesis_type)?.name} · {p.status}
                </p>
                {p.localDir && (
                  <p className="text-[10px] text-white/15 mt-1 font-mono truncate"><Icon name="📁" className="inline-flex align-[-0.125em] mr-1" size={15} />{p.localDir}</p>
                )}
              </Link>
            ))}
          </div>
