...
export default function WorkspacePage() {
  ...
  const handleLoadProject = async () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        alert(t("workspaceDetail.readFileError"));
      }
    };
    input.click();
  };
  ...
  const handleSelectDir = async () => {
    ...
    if (typeof window !== "undefined" && "showDirectoryPicker" in window) {
      ...
      const dir = prompt(t("workspaceDetail.localDirPrompt"), "~/Documents/Research");
        ...
        {showCreate && (
          <div className="mb-6 p-6 rounded-[18px] bg-[#13161e] border border-white/[0.04]">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-[15px] text-white/30 mb-2">{t("workspaceDetail.localDirLabel")}</label>
                <div className="flex gap-2">
                  ...
                  <button onClick={handleSelectDir} className="px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-[16px] text-white/50 hover:text-white/70 whitespace-nowrap">
                    <Icon name="📁" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("workspaceDetail.selectBtn")}
                  </button>
                </div>
                <p className="text-[14px] text-white/20 mt-1">{t("workspaceDetail.localDirHint")}</p>
              </div>
              <div className="flex gap-2">
