...
export default function AnalyzerPage() {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let workFile = file;
    if (isHwpFile(file)) {
      const hwpTok = aiProgress.begin(t("analyzerPage.hwpConverting"));
      const conv = await convertHwpToPdf(file, (m) => aiProgress.setLabel(m));
      aiProgress.end(hwpTok);
      if (conv.pdf) {
        workFile = conv.pdf;
        setUodeMeta(t("analyzerPage.hwpConvertedMsg"));
      } else if (conv.reason === "failed" && conv.message) {
        setUodeMeta(`${t("analyzerPage.hwpConverterUnavailable")} (${conv.message})`);
      }
    }
    ...
    if (
      ...
      if (!pdfJsLoaded) {
        alert(t("analyzerPage.pdfEngineLoading"));
        e.target.value = "";
        return;
      }
      ...
      reader.onload = async (ev) => {
        ...
        try {
          ...
          alert(t("analyzerPage.pdfRenderFailed"));
        } finally {
      ...
      try {
        ...
        setUodeMeta(
          `${formatLabel(doc.format)} · ${doc.charCount.toLocaleString()}${t("analyzerPage.charsUnit")} · ` +
            `${doc.pageCount}p · ${t("analyzerPage.confidenceLabel")} ${doc.confidence}%${note}`
        );
        setDocText(toFormattedText(doc));
      } catch (err: any) {
        setDocText(
          `[${workFile.name}]\n${t("analyzerPage.docExtractFailed")}: ${
            err?.message || t("analyzerPage.unknownError")
          }`
        );
      } finally {
        setRenderProgress(null);
      ...
      setDocText(`[${workFile.name}]\n${t("analyzerPage.unsupportedFormat")}`);
    }
    e.target.value = "";
  };
  ...
  const pasteText = () => {
    ...
    if (text) {
      setFileName(t("analyzerPage.pastedTextLabel"));
      setIsPdf(false);
      setPdfPages([]);
      setPdfDoc(null);
      setUodeBlocks([]);
    ...
    try {
      ...
      const msg =
        e instanceof Error && e.message === "API_KEY_MISSING"
          ? t("analyzerPage.apiKeyMissing")
          : t("analyzerPage.analysisError");
  ...
  return (
    ...
    >
      ...
      <div className="lg:hidden flex items-center gap-1 px-2 py-1.5 border-b border-white/[0.04] bg-[#13161e] flex-shrink-0">
        {([
          { id: "viewer", icon: "litReview", label: t("analyzerPage.originalConverted") },
          { id: "analysis", icon: "analyzer", label: t("analyzerPage.aiAnalysis") },
        ] as const).map((p) => (
          <button
            key={p.id}
      ...
      >
          ...
          {renderProgress && (
            <div className="mb-2">
              <div className="flex justify-between text-[12px] text-white/30 mb-1">
                <span>{t("analyzerPage.loading")}</span>
                <span>
        ...
        {/* 페이지 썸네일 */}
        <div className="flex-1 overflow-y-auto px-2 pb-3 min-w-0">
          {pdfPages.length > 0 ? (
            <>
              <p className="text-[13px] font-semibold text-white/30 mb-1.5 px-1">
                <Icon name="📑" className="inline-flex align-[-0.125em] mr-1" size={15} />
                {pdfPages.length}{t("analyzerPage.pagesUnit")}
              </p>
            ...
            <p className="text-[12px] text-white/20 px-1 leading-relaxed">
              {uodeMeta || t("analyzerPage.textLoadedHint")}
            </p>
          ) : (
            <p className="text-[12px] text-white/15 px-1">
              {t("analyzerPage.noDocPreviewHint")}
            </p>
          )}
        </div>
      </div>
      ...
      >
        ...
        <div className="border-b border-white/[0.04] flex flex-wrap items-center gap-1.5 px-3 py-2 lg:h-10 lg:flex-nowrap lg:py-0 flex-shrink-0">
          <p className="text-[15px] font-semibold text-white/60 flex items-center gap-1">
            <Icon name="litReview" size={14} />{t("analyzerPage.originalViewer")}
          </p>
          {pdfPages.length > 0 && (
          ...
          <div className="flex items-center gap-1.5 flex-wrap ml-auto lg:ml-0">
            ...
            >
              ...
              {loading ? t("analyzer.analyzing") : t("analyzerPage.autoAnalyze")}
            </button>
        ...
        {/* 본문 */}
        <div className="flex-1 overflow-y-auto min-w-0" style={{ fontSize }}>
          {renderProgress && (
            <div className="flex flex-col items-center justify-center h-full text-white/30">
              <p className="text-[35px] mb-4 animate-pulse">
                <Icon name="📄" className="inline-flex align-[-0.125em] mr-1" size={15} />
              </p>
              <p className="text-[17px] mb-2">{t("analyzerPage.loadingEllipsis")}</p>
                ...
                <div key={pg.pageNum} id={`apage-${pg.pageNum}`}>
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-[13px] text-white/25 bg-white/[0.04] px-2 py-0.5 rounded">
                      {t("analyzerPage.pagePrefix")} {pg.pageNum}
                    </span>
                  ...
                  {pg.text && (
                    <div className="max-w-[640px] mx-auto mt-3">
                      <div className="flex items-center gap-2 mb-1.5">
                        <div className="flex-1 h-px bg-white/[0.04] min-w-0" />
                        <span className="text-[12px] text-white/15">{t("analyzerPage.convertedText")}</span>
                        <div className="flex-1 h-px bg-white/[0.04] min-w-0" />
                      </div>
      ...
      >
        <div className="p-3 border-b border-white/[0.04] flex-shrink-0">
          <p className="text-[15px] font-semibold text-white/60 mb-2 flex items-center gap-1">
            <Icon name="analyzer" size={14} />{t("analyzerPage.aiAnalysis")}
          </p>
          ...
          >
            ...
            {loading
              ...
              : `${ANALYSIS_TABS.find((x) => x.id === tab)?.label} ${t("analyzerPage.run")}`}
          </button>
        </div>
            ...
            !loading && (
              <div className="text-center py-16 text-white/15">
                ...
                <p className="text-[15px]">
                  {hasDoc ? t("analyzerPage.selectAnalysisTypeHint") : t("analyzer.empty")}
                </p>
              </div>
            )
          )}
