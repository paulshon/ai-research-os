...
function coord(n: { page: number; paragraph: number; sentence: number }, t: (k: string) => string) {
  return `${n.page}p · ${n.paragraph}${t("validationPage.paraUnit")} · ${n.sentence}${t("validationPage.sentUnit")}`;
}
...
export default function ValidationPage() {
  ...
  const TABS: { id: VTab; label: string; icon: string }[] = [
    { id: "plagiarism", label: t("validationPage.tabPlagiarism"), icon: "plagiarism" },
    { id: "grammar", label: t("validationPage.tabGrammar"), icon: "spell" },
    { id: "ai", label: t("validationPage.tabAi"), icon: "aiscan" },
    { id: "deep", label: t("validationPage.tabDeep"), icon: "shieldCheck" },
    { id: "report", label: t("validationPage.tabReport"), icon: "checklist" },
  ];
  ...
  return (
    <div className="p-4 md:p-5 lg:px-6 lg:py-6 font-nanum-gothic">
      ...
      <div className="max-w-[1680px] mx-auto flex flex-col xl:flex-row gap-6 items-start">
        ...
        {/* s-renew-12: 제목/부제는 셸 상단 Hero 가 담당 — 중복 제거 */}

        {/* 공유 입력 */}
        <div className="p-4 rounded-2xl bg-[#13161e] border border-white/[0.04] mb-4">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[15px] font-semibold text-white/70">{t("validationPage.targetBody")}</p>
            {analyzed && (
              <span className="text-[13px] text-white/30">
                {t("validationPage.sentPageStat").replace("{count}", String(sentenceMap.length)).replace("{pages}", String(pageStats.length))}
              </span>
            )}
          </div>
          <textarea
            ...
            placeholder={t("validationPage.bodyPlaceholder")}
          />
          <button
            onClick={runAllLocal}
          ...
          >
            {analyzed ? t("validationPage.reanalyze") : t("validationPage.runAll")}
          </button>
          {analyzed && (
            <span className="ml-3 text-[14px] text-white/30">
              {t("validationPage.statLine").replace("{p}", String(plagOverall)).replace("{g}", String(gScore)).replace("{a}", String(aiOverall))}
            </span>
          )}
        </div>
        ...
        {tab === "plagiarism" && (
          <div>
            {!analyzed ? (
              <EmptyHint icon="plagiarism" text={t("validationPage.emptyHintRun")} />
            ) : (
              <>
                <ScoreHeader label={t("validationPage.overallSimilarity")} value={`${plagOverall}%`} color={riskColor(plagOverall ?? 0)}
                  sub={(plagOverall ?? 0) < 15 ? t("validationPage.low") : (plagOverall ?? 0) < 30 ? t("validationPage.caution") : t("validationPage.high")} />
                <PageBars stats={pageStats} field="plag" label={t("validationPage.pageSimilarity")} pageUnit={t("validationPage.pageUnit")} />
                <SentenceList
                  title={t("validationPage.similarSentences")}
                  empty={t("validationPage.noPlagFound")}
                  items={plag.map((p) => ({
                    coord: coord(p, t),
                    metric: `${p.similarity}%`,
                    color: riskColor(p.similarity),
                    text: sentenceMap.find((s) => s.id === p.sentenceId)?.text ?? "",
                    note: p.plagiarismType === "internal" ? `${t("validationPage.typeInternalDup")}${p.note}`
                      : p.plagiarismType === "uncited-quote" ? `${t("validationPage.typeUncited")}${p.note}` : p.note,
                  }))}
                />
                <p className="text-[13px] text-white/20 mt-3">
                  {t("validationPage.localEngineNote")}
                </p>
              </>
            )}
          </div>
        ...
        {tab === "grammar" && (
          <div>
            {!analyzed ? (
              <EmptyHint icon="spell" text={t("validationPage.emptyHintRun")} />
            ) : (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-5">
                  <StatCard label={t("validationPage.accuracy")} value={`${gScore}${t("validationPage.pointsUnit")}`} color="#5ebd7c" />
                  <StatCard label={t("validationPage.errorsLabel")} value={`${grammar.filter((g) => g.severity === "error").length}${t("validationPage.itemsUnit")}`} color="#ff7066" />
                  <StatCard label={t("validationPage.warningsLabel")} value={`${grammar.filter((g) => g.severity === "warning").length}${t("validationPage.itemsUnit")}`} color="#e8b84b" />
                  <StatCard label={t("validationPage.recommendLabel")} value={`${grammar.filter((g) => g.severity === "info").length}${t("validationPage.itemsUnit")}`} color="#6c8cff" />
                </div>
                <SentenceList
                  title={t("validationPage.correctionSuggestions")}
                  empty={t("validationPage.noGrammarIssues")}
                  items={grammar.map((g) => ({
                    coord: coord(g, t),
                    metric: g.category,
        ...
        {tab === "ai" && (
          <div>
            {!analyzed ? (
              <EmptyHint icon="aiscan" text={t("validationPage.emptyHintRun")} />
            ) : (
              <>
                <ScoreHeader label={t("validationPage.aiGenRate")} value={`${aiOverall}%`} color={riskColor(aiOverall ?? 0)}
                  sub={(aiOverall ?? 0) < 40 ? t("validationPage.humanDominant") : (aiOverall ?? 0) < 70 ? t("validationPage.mixedStyle") : t("validationPage.aiLikely")} />
                <PageBars stats={pageStats} field="ai" label={t("validationPage.pageGenRate")} pageUnit={t("validationPage.pageUnit")} />
                <SentenceList
                  title={t("validationPage.riskSentences")}
                  empty={t("validationPage.noAiSuspect")}
                  items={[...ai]
                    ...
                    .map((a) => ({
                      ...
                      note: `${t("validationPage.causePrefix")}${a.reasons.join(" · ")}  |  Perplexity ${a.perplexity} · Burstiness ${a.burstiness} · Entropy ${a.entropy}`,
                    }))}
                />
              </>
            )}
        ...
        {tab === "report" && (
          <div>
            {!analyzed || !grade ? (
              <EmptyHint icon="checklist" text={t("validationPage.reportEmptyHint")} />
            ) : (
              <>
                <div className="p-6 rounded-2xl bg-[#13161e] border border-white/[0.06] mb-5 flex items-center gap-6">
                  <div className="text-center">
                    ...
                    <p className="text-[14px] text-white/30 mt-1">{t("validationPage.finalGrade")}</p>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[15px] text-white/60">{t("validationPage.totalScore")}</span>
                      <span className="text-[19px] font-bold text-[#6c8cff]">{grade.total}{t("validationPage.pointsUnit")}</span>
                    </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                  <StatCard label={t("validationPage.plagScore")} value={`${plagOverall}%`} color={riskColor(plagOverall ?? 0)} sub={t("validationPage.lowerBetter")} />
                  <StatCard label={t("validationPage.grammarScore")} value={`${gScore}${t("validationPage.pointsUnit")}`} color="#5ebd7c" sub={t("validationPage.higherBetter")} />
                  <StatCard label={t("validationPage.aiRisk")} value={`${aiOverall}%`} color={riskColor(aiOverall ?? 0)} sub={t("validationPage.lowerBetter")} />
                  <StatCard label={t("validationPage.styleReadability")} value={`${Math.max(0, 100 - grammar.filter(g => g.category === "style").length * 6)}${t("validationPage.pointsUnit")}`} color="#a78bfa" />
                </div>
                <div className="mt-5 p-4 rounded-xl bg-[#13161e] border border-white/[0.04]">
                  <p className="text-[15px] font-semibold mb-2">{t("validationPage.fixPriority")}</p>
                  <ol className="space-y-1.5 text-[15px] text-white/60 list-decimal pl-5">
                    {(plagOverall ?? 0) >= 30 && <li>{t("validationPage.fixPlag").replace("{n}", String(plag.length))}</li>}
                    {grammar.filter(g => g.severity === "error").length > 0 && <li>{t("validationPage.fixGrammar").replace("{n}", String(grammar.filter(g => g.severity === "error").length))}</li>}
                    {(aiOverall ?? 0) >= 60 && <li>{t("validationPage.fixAi")}</li>}
                    {grammar.filter(g => g.category === "style").length > 0 && <li>{t("validationPage.fixStyle").replace("{n}", String(grammar.filter(g => g.category === "style").length))}</li>}
                    <li>{t("validationPage.fixDeep")}</li>
                  </ol>
                </div>
              </>
            )}
        ...
        <aside className="w-full xl:w-[320px] flex-shrink-0 xl:sticky xl:top-4 space-y-3">
          <div className="p-4 rounded-2xl bg-[#13161e] border border-white/[0.05]">
            <p className="text-[13px] font-semibold text-white/70 mb-3 flex items-center gap-1.5">
              <Icon name="chart" size={15} /> {t("validationPage.qualitySnapshot")}
            </p>
            {!analyzed || !grade ? (
              <p className="text-[12px] text-white/30 leading-relaxed py-2">
                {t("validationPage.snapshotHint")}
              </p>
            ) : (
              <>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-white/35">{t("validationPage.totalQualityScore")}</p>
                    <p className="text-[22px] font-bold text-[#6c8cff] leading-tight">{grade.total}<span className="text-[13px] text-white/30 font-normal">{t("validationPage.pointsUnit")}</span></p>
                  </div>
                </div>
                <div className="space-y-2.5">
                  {[
                    { label: t("validationPage.mPlagSim"), v: plagOverall ?? 0, good: "low", unit: "%" },
                    { label: t("validationPage.mGrammarAcc"), v: gScore ?? 0, good: "high", unit: t("validationPage.pointsUnit") },
                    { label: t("validationPage.mAiRate"), v: aiOverall ?? 0, good: "low", unit: "%" },
                  ].map((m) => {
                    ...
                    return (
                      <div key={m.label}>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-[12px] font-semibold tabular-nums" style={{ color: c }}>{m.v}{m.unit}</span>
                        </div>
          ...
          {analyzed && pageStats.length > 0 && (
            <div className="p-4 rounded-2xl bg-[#13161e] border border-white/[0.05]">
              <p className="text-[13px] font-semibold text-white/70 mb-3">{t("validationPage.pageRisk")}</p>
                ...
                {pageStats.slice(0, 8).map((s) => {
                  ...
                  return (
                    <div key={s.page} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 h-1.5 bg-[#1a1e2a] rounded-full overflow-hidden min-w-0">
          ...
          {analyzed && grade && (
            <div className="p-4 rounded-2xl bg-[#13161e] border border-white/[0.05]">
              <p className="text-[13px] font-semibold text-white/70 mb-2.5">{t("validationPage.aiDiagSummary")}</p>
              <ul className="space-y-1.5 text-[12px] text-white/55">
                <li className="flex items-start gap-1.5"><span className="text-[#6c8cff] mt-0.5">→</span><span>{t("validationPage.diag1").replace("{p}", String(plag.length)).replace("{g}", String(grammar.filter(g=>g.severity==="error").length))}</span></li>
                <li className="flex items-start gap-1.5"><span className="text-[#6c8cff] mt-0.5">→</span><span>{t("validationPage.diag2").replace("{n}", String(ai.filter(a=>a.aiProbability>=60).length))}</span></li>
                <li className="flex items-start gap-1.5"><span className="text-[#6c8cff] mt-0.5">→</span><span>{t("validationPage.diag3")}</span></li>
              </ul>
            </div>
          )}
        </aside>
...
function PageBars({ stats, field, label, pageUnit }: { stats: { page: number; plag: number; ai: number }[]; field: "plag" | "ai"; label: string; pageUnit: string }) {
        ...
        {stats.map((s) => {
          ...
          return (
            <div key={s.page} className="flex items-center gap-3">
              <span className="text-[14px] text-white/40 w-12">{s.page}{pageUnit}</span>
              <div className="flex-1 h-2 bg-[#1a1e2a] rounded-full overflow-hidden min-w-0">
