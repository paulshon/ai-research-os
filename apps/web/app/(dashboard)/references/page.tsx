  ...
  async function handleImport() {
    ...
    try {
      if (importMode === "doi") {
        ...
        const parsed = parseBibTextToRefEntry(input.trim());
        if (!parsed) {
          flash("error", `${importMode.toUpperCase()} ${t("references.parseFailed")}`);
        } else {
          addRefs([parsed.ref]);
          flash("ok", `${parsed.format.toUpperCase()} ${t("references.imported")}`);
          setInput("");
        }
      }
    } catch {
    ...
    <div className="p-4 md:p-6 lg:p-8 font-nanum-gothic">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="font-nanum-myeongjo text-[22px] font-bold text-[#e8eaf0]">
            {t("references.title")}
          </h1>
        </div>
        <p className="text-[13px] text-white/35 mb-6">{t("references.subtitle")}</p>

        {msg && (
          <div
            className={`mb-5 px-4 py-2.5 rounded-lg text-[12px] flex items-center gap-2 ${
              msg.type === "ok"
        ...
        <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04] mb-5">
          ...
          <div className="flex flex-wrap items-center gap-1.5 mb-3">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-[11.5px] text-white/40 border border-white/[0.06] hover:text-white/70 transition-all flex items-center gap-1"
            >
          ...
          <textarea
            ...
            className="w-full px-3 py-2.5 bg-[#1a1e2a] border border-white/[0.06] rounded-lg text-[12px] text-white placeholder:text-white/20 focus:outline-none focus:border-[#3ecfb2] min-h-[80px] resize-y font-mono leading-relaxed"
        ...
        <div className="p-4 rounded-[16px] bg-[#13161e] border border-white/[0.04] mb-5 flex flex-col lg:flex-row lg:items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] text-white/30 mr-1">{t("references.sortBy")}</span>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[11px] text-white/30 mr-1">{t("references.style")}</span>
          ...
          <div className="lg:ml-auto flex items-center gap-1.5">
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-[11px] text-white/50 border border-white/[0.06] hover:text-white/80 hover:bg-white/[0.04] transition-all disabled:opacity-30 flex items-center gap-1"
            >
            ...
            <button
              onClick={exportTxt}
              disabled={refDB.length === 0}
              className="px-3 py-1.5 rounded-lg text-[11px] bg-[#6c8cff]/15 border border-[#6c8cff]/30 text-[#6c8cff] hover:bg-[#6c8cff]/25 transition-all disabled:opacity-30 flex items-center gap-1"
            >
        ...
        <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04]">
          <div className="flex items-center justify-between mb-4">
            ...
            {refDB.length > 0 && (
              <button
                ...
                className="text-[11px] text-white/30 hover:text-[#ff7066] transition-colors flex items-center gap-1"
              >
          ...
          {sortedRefs.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-[200px] text-white/15 text-center px-4">
              ...
              <p className="text-[13px] mt-3">{t("references.emptyTitle")}</p>
              <p className="text-[11px] mt-1 text-white/10">{t("references.emptyHint")}</p>
            </div>
          ) : (
            <ol className="space-y-2.5">
              {sortedRefs.map((r: RefEntry, i: number) => (
                ...
                >
                  <span className="text-[11px] text-white/25 mt-0.5 tabular-nums w-6 text-right flex-shrink-0">
                    {i + 1}.
                  </span>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      {r.year && <span className="text-[10px] text-white/25">{r.year}</span>}
                      {r.doi && (
                        <a
