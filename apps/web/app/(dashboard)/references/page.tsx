...
export default function ReferencesPage() {
    ...
    try {
      if (importMode === "doi") {
        ...
        if (refs.length === 0) {
          ...
          addRefs(refs);
          flash("ok", `${importMode.toUpperCase()} ${refs.length}${t("referencesPage.countUnit")} ${t("references.imported")}`);
          setInput("");
        }
      }
    } catch {
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      ...
      if (bibCount > 0 && docFiles.length === 0) {
        flash("ok", t("referencesPage.bibImported").replace("{count}", String(bibCount)));
      } else if (bibCount > 0) {
        flash("ok", t("referencesPage.bibAndDocs").replace("{count}", String(bibCount)).replace("{docs}", String(docFiles.length)));
  ...
  function buildText(): string {
    ...
    const header = `${t("referencesPage.refListTitle")} (${styleMeta?.name ?? citationStyle}) — ${sortedRefs.length}${t("referencesPage.countUnit")}\n${"=".repeat(40)}\n\n`;
  ...
  const placeholder =
    importMode === "doi"
      ? t("referencesPage.doiPlaceholder")
      : importMode === "ris"
      ? t("referencesPage.risPlaceholder")
      : t("referencesPage.bibtexPlaceholder");
