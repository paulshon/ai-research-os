...
const PIPELINE_STEPS = [
  { key: "collect",  icon: "📥", colorFrom: "#4a6cf7", colorTo: "#6c8cff", gemini: false },
  { key: "metadb",   icon: "🗄️", colorFrom: "#3ecfb2", colorTo: "#34d399", gemini: false },
  { key: "vector",   icon: "🔍", colorFrom: "#e8b84b", colorTo: "#f59e0b", gemini: false },
  { key: "understand", icon: "🧠", colorFrom: "#a78bfa", colorTo: "#8b5cf6", gemini: true },
  { key: "reason",   icon: "💡", colorFrom: "#ec4899", colorTo: "#f472b6", gemini: true },
];
...
export default function LiteratureReviewPage() {
  ...
  const TABS = useMemo(() => [
    ...
    { id: "pipeline",  label: t("litReview.tabPipeline"),  icon: "⚙️" },
  ...
  const handleSearch = async (regionOverride?: "domestic" | "international") => {
    ...
    try {
      const res = await fetch(
        `/api/scholar?q=${encodeURIComponent(searchQuery)}&region=${region}&limit=30`
      );
    ...
    <div className="flex flex-col md:flex-row font-nanum-gothic">
      ...
      <div className="md:hidden flex items-center gap-2 px-3 py-2 border-b border-white/[0.04] bg-[#0d0f14] sticky top-0 z-[40]">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#1a1e2a] border border-white/[0.06] text-[12px] text-white/70 active:scale-95 transition-transform"
        >
          <Icon name="🔬" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("litReview.title")}
        </button>
        <span className="text-[10px] text-white/30">{t("litReview.selectedCount")}: {selectedDBs.length} DBs</span>
      </div>
      ...
      <div className={`${mobileSidebar ? "fixed inset-y-0 left-0 z-[7900] flex animate-slide-in-left" : "hidden"} md:relative md:z-auto md:flex w-80 max-w-[85vw] border-r border-white/[0.04] bg-[#0d0f14] flex-col overflow-y-auto flex-shrink-0`}>
        ...
        <div className="md:hidden flex justify-end px-3 pt-2 -mb-2">
          <button
            ...
            className="text-white/40 hover:text-white/70 text-[20px] leading-none px-2 py-1"
            aria-label="닫기"
          >
            ×
          </button>
        ...
        <div className="p-4 border-b border-white/[0.04]">
          ...
          <p className="text-[11px] text-white/25 mt-1">{t("litReview.desc")}</p>
        </div>
        ...
        <div className="p-3 border-b border-white/[0.04]">
          <div className="flex gap-1.5">
            <input
              ...
              className="flex-1 px-3 py-2 rounded-lg bg-[#1a1e2a] border border-white/[0.06] text-[12px] text-white/70 focus:border-[#6c8cff] focus:outline-none"
            />
            <button
              onClick={() => handleSearch()}
              disabled={searchLoading || !searchQuery.trim()}
              className="px-3 py-2 bg-[#4a6cf7] text-white rounded-lg text-[12px] font-medium disabled:opacity-30 whitespace-nowrap"
            >
          ...
          {searchQuery.trim() && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {ACADEMIC_DBS.filter((db) => KOREAN_DB_IDS.has(db.id) && selectedDBs.includes(db.id)).slice(0, 8).map((db) => (
                <a key={db.id} href={buildDbSearchUrl(db, searchQuery)} target="_blank" rel="noopener" className="text-[10px] px-2 py-1 rounded bg-[#6c8cff]/10 text-[#6c8cff] hover:bg-[#6c8cff]/20">
                  {db.name} 검색
                </a>
              ))}
            </div>
        ...
        <div className="overflow-y-auto p-2 max-h-[50vh] md:max-h-none md:flex-1">
          <p className="text-[11px] text-white/30 px-2 mb-1.5">{t("litReview.dbBrowser")}</p>
            ...
            return (
              <div key={cat.id} className="mb-1">
                <button
                  ...
                  className="w-full flex items-center justify-between px-2.5 py-2 rounded-lg text-[12px] hover:bg-white/[0.03] transition-colors"
                >
                  <span className="text-white/60">{t(cat.labelKey)}</span>
                  <span className="flex items-center gap-2">
                    <span className="text-[10px] text-white/25">{selectedInCat}/{catDBs.length}</span>
                    <span className="text-[10px] text-white/20">{isExpanded ? <Icon name="chevronDown" size={11} /> : <Icon name="chevronRight" size={11} />}</span>
                  </span>
                </button>
                {isExpanded && (
                  <div className="ml-2 pl-2 border-l border-white/[0.04]">
                    <button
                      ...
                      className="text-[10px] text-[#6c8cff] hover:text-[#8ba5ff] px-2 py-1 transition-colors"
                    >
                    ...
                    {catDBs.map(db => (
                      <a
                        ...
                        className="flex items-center gap-2 px-2 py-1.5 rounded text-[11px] hover:bg-white/[0.04] group"
                      >
                        <span className="text-white/50 group-hover:text-white/80 flex-1 min-w-0 truncate transition-colors">
                          {db.name}
                        </span>
                        {db.apiAvailable && (
                          <span className="text-[8px] px-1 py-0.5 rounded bg-[#3ecfb2]/10 text-[#3ecfb2] flex-shrink-0">API</span>
                        )}
                      </a>
                    ))}
                  </div>
        ...
        <div className="h-10 border-b border-white/[0.04] flex items-center px-3 gap-0.5 flex-shrink-0 overflow-x-auto">
          {TABS.map(tab => (
            <button
              ...
              className={`px-3 py-1 rounded-md text-[12px] transition-colors whitespace-nowrap flex-shrink-0 ${
                activeTab === tab.id
          ...
          {loading && (
            <span className="ml-auto text-[11px] text-[#e8b84b] animate-pulse flex-shrink-0">
        ...
        <div className="overflow-y-auto p-4 md:p-6">
            ...
            <div className="max-w-4xl">
              ...
              <div className="flex items-center gap-1 mb-5">
                ...
                <div className="ml-auto flex items-center gap-2">
                  <span className="text-[10px] text-white/15">{t("litReview.realApiNote")}</span>
                </div>
              </div>
              ...
              <div className="mb-5">
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) => e.key === "Enter" && handleSearch()}
                    placeholder={t("litReview.searchPlaceholder")}
                    className="flex-1 px-4 py-2.5 rounded-xl bg-[#13161e] border border-white/[0.06] text-[13px] text-white/80 placeholder:text-white/20 focus:border-[#6c8cff] focus:outline-none"
                  />
                  <button
                    onClick={() => handleSearch()}
                    disabled={searchLoading || !searchQuery.trim()}
                    className="px-5 py-2.5 bg-[#4a6cf7] text-white rounded-xl text-[13px] font-medium disabled:opacity-30 whitespace-nowrap active:scale-95 transition-transform"
                  >
                    {searchLoading ? "..." : t("litReview.searchBtn")}
                  </button>
                </div>
                <div className="flex items-center justify-between mt-2">
                  <span className="text-[10px] text-white/25">{t("litReview.selectedCount")}: {selectedDBs.length} DBs</span>
                  <button
                    onClick={() => setMobileSidebar(true)}
                    className="md:hidden text-[10px] text-[#6c8cff] flex items-center gap-1"
                  >
              ...
              {searchLoading ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-white/30 text-[13px]">{t("litReview.searching")}</p>
                  <p className="text-white/15 text-[11px] mt-1">{t("litReview.searchingReal")}</p>
                </div>
              ) : (() => {
                            ...
                            >
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  <h4 className="text-[13px] font-medium text-white/80 leading-tight mb-1">
                                    {paper.title}
                                  </h4>
                                  ...
                                  {paper.abstract && (
                                    <p className="text-[11px] text-white/30 leading-relaxed mb-2 line-clamp-3">
                                      {paper.abstract}
                                    </p>
                                  )}
                                  {/* 링크 — 실제 DOI/원문 */}
                                  <div className="flex items-center gap-2 mb-1.5">
                                    {paper.url && paper.url !== "#" && (
                                      <a href={paper.url} target="_blank" rel="noopener" onClick={e => e.stopPropagation()} className="text-[10px] hover:underline flex items-center gap-0.5" style={{ color: regionColor }}>
                                        <Icon name="🔗" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("litReview.viewPaper")}
                                      </a>
                                    )}
                                    {paper.doi && (
                                      <a href={`https://doi.org/${paper.doi}`} target="_blank" rel="noopener" onClick={e => e.stopPropagation()} className="text-[10px] text-[#a78bfa] hover:underline">
                                        DOI: {paper.doi.length > 30 ? paper.doi.slice(0, 30) + "…" : paper.doi}
                                      </a>
                                    )}
                                  </div>
                                  <div className="flex items-center gap-3 flex-wrap">
                                    <span className="text-[10px] text-white/20"><Icon name="📊" className="inline-flex align-[-0.125em] mr-1" size={15} />{paper.citations} {t("litReview.cited")}</span>
                                    <span className="text-[10px] text-white/20"><Icon name="📂" className="inline-flex align-[-0.125em] mr-1" size={15} />{paper.source}</span>
                                    {paper.similarity != null && (
                                      <span className="text-[10px]" style={{ color: "#3ecfb2" }}>
                                        ≈ {(paper.similarity * 100).toFixed(0)}% {t("litReview.match")}
                                      </span>
                                    )}
                                    {paper.keywords?.slice(0, 3).map(kw => (
                                      <span key={kw} className="text-[9px] px-1.5 py-0.5 rounded bg-white/[0.04] text-white/25">
                                        {kw}
                                      </span>
                                    ))}
                                  </div>
                ...
                return (
                  <div className="text-center py-20 text-white/15">
                    <p className="text-[40px] mb-3">{searchRegion === "domestic" ? <Icon name="flag" size={36} /> : <Icon name="globe" size={36} />}</p>
                    <p className="text-[15px] mb-1">{t("litReview.emptyTitle")}</p>
                    <p className="text-[12px] text-white/10">{t("litReview.emptyDesc")}</p>
                    <p className="text-[10px] text-white/8 mt-2">{t("litReview.realApiNote")}</p>
                  </div>
                );
              })()}
            </div>
          ...
          {activeTab === "databases" && (
            <div className="max-w-5xl">
              <div className="flex items-center gap-3 mb-5 flex-wrap">
                <h3 className="text-[15px] font-semibold">{t("litReview.dbTitle")} (53)</h3>
                <select
                  value={filterField}
                  onChange={e => setFilterField(e.target.value)}
                  className="px-2.5 py-1.5 rounded-lg bg-[#1a1e2a] border border-white/[0.06] text-[11px] text-white/50"
                >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between mb-1.5">
                        <h4 className="text-[12px] font-medium text-white/70 truncate flex-1">{db.name}</h4>
                        <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                          {db.apiAvailable && (
                            <span className="text-[8px] px-1 py-0.5 rounded bg-[#3ecfb2]/10 text-[#3ecfb2]">API</span>
                          )}
                        </div>
                      </div>
                      <p className="text-[10px] text-white/25 mb-1">{db.feature}</p>
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-white/[0.03] text-white/20">{db.type}</span>
                        <span className="text-[9px] text-white/15">{db.field}</span>
                        <span className="text-[9px] text-white/15 ml-auto">{db.provider}</span>
                      </div>
                    </a>
                  );
                })}
          ...
          {activeTab === "pipeline" && (
            <div className="max-w-3xl mx-auto">
              <h3 className="text-[15px] font-semibold mb-2">{t("litReview.pipelineTitle")}</h3>
              <p className="text-[12px] text-white/30 mb-8">{t("litReview.pipelineDesc")}</p>
                ...
                {PIPELINE_STEPS.map((step, idx) => (
                  <div key={step.key} className="flex items-start gap-4">
                    ...
                    <div className="flex flex-col items-center flex-shrink-0">
                      ...
                      >
                        <Icon name={step.icon} className="inline-flex align-[-0.125em]" size={15} />
                      </div>
                      {idx < PIPELINE_STEPS.length - 1 && (
                        <div className="w-0.5 h-8 bg-white/[0.06] mt-2" />
                      )}
                    </div>
                    ...
                    <div className="flex-1 pt-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="text-[13px] font-medium text-white/80">
                          {t(`litReview.pipe_${step.key}`)}
                        </h4>
                        {step.gemini ? (
                          <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#e8b84b]/10 text-[#e8b84b]">
                            Gemini
                          </span>
                        ) : (
                          <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#3ecfb2]/10 text-[#3ecfb2]">
                            {t("litReview.localCpu")}
                          </span>
                        )}
                      </div>
                      <p className="text-[11px] text-white/30 leading-relaxed">
                        {t(`litReview.pipe_${step.key}_desc`)}
                      </p>
                      <p className="text-[10px] text-white/15 mt-1">
                        {t(`litReview.pipe_${step.key}_tech`)}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              {/* 비용 구조 요약 */}
              <div className="mt-10 p-5 rounded-xl border border-white/[0.04] bg-[#13161e]">
                <h4 className="text-[13px] font-semibold mb-3">{t("litReview.costTitle")}</h4>
                <div className="grid grid-cols-2 gap-2">
                  {["collect", "metadb", "vector"].map(k => (
                    <div key={k} className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/[0.02]">
                      <span className="text-[11px] text-white/40">{t(`litReview.pipe_${k}`)}</span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#3ecfb2]/10 text-[#3ecfb2]">{t("litReview.free")}</span>
                    </div>
                  ))}
                  {["understand", "reason"].map(k => (
                    <div key={k} className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/[0.02]">
                      <span className="text-[11px] text-white/40">{t(`litReview.pipe_${k}`)}</span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#e8b84b]/10 text-[#e8b84b]">{t("litReview.minimal")}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="max-w-3xl">
              {researchGap ? (
                <>
                  ...
                  <div className="text-[13px] text-white/70 leading-[1.8] whitespace-pre-wrap">
                    {researchGap}
                  </div>
                </>
              ) : loading ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-white/30 text-[13px]">{t("litReview.analyzingGap")}</p>
                </div>
              ) : (
                <div className="text-center py-20 text-white/15">
                  <p className="text-[40px] mb-3"><Icon name="💡" className="inline-flex align-[-0.125em] mr-1" size={15} /></p>
                  <p className="text-[14px]">{t("litReview.gapEmpty")}</p>
                  <p className="text-[12px] text-white/10 mt-1">{t("litReview.gapEmptyDesc")}</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="max-w-3xl">
              {researchDesign ? (
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-white/30 text-[13px]">{t("litReview.analyzingDesign")}</p>
                </div>
              ) : (
                <div className="text-center py-20 text-white/15">
                  <p className="text-[40px] mb-3"><Icon name="📐" className="inline-flex align-[-0.125em] mr-1" size={15} /></p>
                  <p className="text-[14px]">{t("litReview.designEmpty")}</p>
                  <p className="text-[12px] text-white/10 mt-1">{t("litReview.designEmptyDesc")}</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="max-w-3xl">
              {clusterAnalysis ? (
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-white/30 text-[13px]">{t("litReview.analyzingCluster")}</p>
                </div>
              ) : (
                <div className="text-center py-20 text-white/15">
                  <p className="text-[40px] mb-3"><Icon name="🔬" className="inline-flex align-[-0.125em] mr-1" size={15} /></p>
                  <p className="text-[14px]">{t("litReview.clusterEmpty")}</p>
                  <p className="text-[12px] text-white/10 mt-1">{t("litReview.clusterEmptyDesc")}</p>
                </div>
              )}
            </div>
          )}
      ...
      {/* 플로팅 분석 메뉴 - 데스크탑=우측 상단 / 모바일=하단(바텀네비 위) */}
      <div className="fixed right-3 bottom-20 md:right-6 md:top-[72px] md:bottom-auto z-[60] flex flex-col gap-2 items-end">
        <button
          onClick={analyzeResearchGap}
          disabled={loading || selectedResults.length === 0}
          className="px-3 py-2 bg-gradient-to-r from-[#ec4899] to-[#f472b6] text-white rounded-lg text-[11px] font-medium disabled:opacity-30 transition-all shadow-lg hover:shadow-[#ec4899]/20 hover:-translate-y-0.5 whitespace-nowrap"
        >
          <Icon name="💡" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("litReview.btnGap")}
        </button>
        <button
          onClick={recommendResearchDesign}
          disabled={loading || selectedResults.length === 0}
          className="px-3 py-2 bg-gradient-to-r from-[#4a6cf7] to-[#6c8cff] text-white rounded-lg text-[11px] font-medium disabled:opacity-30 transition-all shadow-lg hover:shadow-[#4a6cf7]/20 hover:-translate-y-0.5 whitespace-nowrap"
        >
          <Icon name="📐" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("litReview.btnDesign")}
        </button>
        <button
          onClick={analyzeCluster}
          disabled={loading || selectedResults.length === 0}
          className="px-3 py-2 bg-gradient-to-r from-[#3ecfb2] to-[#34d399] text-white rounded-lg text-[11px] font-medium disabled:opacity-30 transition-all shadow-lg hover:shadow-[#3ecfb2]/20 hover:-translate-y-0.5 whitespace-nowrap"
        >
          <Icon name="🔬" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("litReview.btnCluster")}
        </button>
      </div>
    </div>
  );
