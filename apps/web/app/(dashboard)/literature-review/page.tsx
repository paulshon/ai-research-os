...
export default function LiteratureReviewPage() {
  ...
  const [graphVisualStyle, setGraphVisualStyle] = useState<GraphVisualStyle>("default");
  const [graphNodeIcon, setGraphNodeIcon] = useState("person");
  const { selectedThesisType, projects, currentProjectId } = useAppStore();
  const currentProject = projects.find((p) => p.id === currentProjectId);
  const projectKeywords = currentProject?.metadata?.keywords ?? [];
  ...
  const toLinkedPapers = useCallback(
    (list: SearchResult[]): LinkedPaper[] =>
      list.map((r) => ({
        title: r.title,
        authors: r.authors,
        year: r.year,
  ...
  // 논문작성 하단 패널로 상세문헌엔진 결과 동기화
  useEffect(() => {
    const selected = searchResults.filter((r) => selectedResults.includes(r.id));
    const papersForList = selected.length > 0 ? selected : searchResults;
    if (papersForList.length > 0) {
      upsertLinkedResult({
        key: "lit-engine-search",
        group: "literature",
        title: t("writingPage.linked.litEngineSearch"),
        body: searchQuery
          ? `검색어: ${searchQuery}\n결과: ${papersForList.length}편${selected.length ? ` (선택 ${selected.length}편)` : ""}`
          : `검색 결과 ${papersForList.length}편`,
        papers: toLinkedPapers(papersForList),
      });
    }
    if (papersForList.length > 0) {
      const { body, papers } = buildNetworkSummaryFromMeta(papersForList);
      upsertLinkedResult({
        key: "lit-network",
        group: "literature",
        title: t("writingPage.linked.litNetwork"),
        body,
        papers,
      });
    }
    if (researchGap.trim()) {
      upsertLinkedResult({
        key: "lit-gap",
        group: "literature",
        title: t("writingPage.linked.litGap"),
        body: researchGap,
        papers: toLinkedPapers(selected.length > 0 ? selected : papersForList),
      });
    }
    if (researchDesign.trim()) {
      upsertLinkedResult({
        key: "lit-design",
        group: "literature",
        title: t("writingPage.linked.litDesign"),
        body: researchDesign,
      });
    }
    if (clusterAnalysis.trim() || theoryConnection.trim()) {
      const body = [clusterAnalysis, theoryConnection].filter((s) => s.trim()).join("\n\n──────────\n\n");
      upsertLinkedResult({
        key: "lit-cluster",
        group: "literature",
        title: t("writingPage.linked.litCluster"),
        body,
        papers: toLinkedPapers(selected.length > 0 ? selected : papersForList),
      });
    }
  ...
  const TABS = useMemo(() => [
    ...
    { id: "analytics", label: t("litReview.tabAnalytics"), icon: "chart" },
    { id: "insights",  label: t("litReview.tabInsights"),  icon: "✨" },
    ...
    try {
      ...
      const kwQuery =
        projectKeywords.length > 0
          ? `&projectKeywords=${encodeURIComponent(projectKeywords.join(","))}`
          : "";
...
Use clear headings and visual indicators. ${locale === "ko" ? "반드시 한국어로 응답하세요." : locale === "zh" ? "请用中文回答。" : "Respond in the same language as the input."}`,
  ...
  // ── v23: 네트워크·분석은 현재 지역 검색 결과 전체 반영 (선택 5편 제한 제거)
  const networkPapers: NetworkPaper[] = useMemo(() => {
    const base = searchResults.filter((r) => r.region === searchRegion);
    return base.map((r) => ({
      id: r.id,
      title: r.title,
      authors: r.authors,
  ...
  }, [searchResults, searchRegion]);

  const selectedPapersForTools = useMemo(() => {
    const selected =
      selectedResults.length > 0
        ? searchResults.filter((r) => selectedResults.includes(r.id))
        : [];
    // 인사이트는 선택분 우선, 없으면 지역 전체
    if (selected.length > 0) return selected;
    return searchResults.filter((r) => r.region === searchRegion);
  }, [searchResults, selectedResults, searchRegion]);

  const analyticsPapers = useMemo(
    () => searchResults.filter((r) => r.region === searchRegion),
    [searchResults, searchRegion],
  );

  const INTEGRATED_SOURCES = useMemo(
    () => [
      { name: "OpenAlex", role: "논문 메타데이터", use: "기본 검색" },
      { name: "Crossref", role: "DOI·서지정보", use: "참고문헌·메타" },
      { name: "Semantic Scholar", role: "논문 관계", use: "유사 논문" },
      { name: "PubMed", role: "의학", use: "의학 분야" },
      { name: "arXiv", role: "프리프린트", use: "최신 연구" },
      { name: "DOAJ", role: "오픈액세스", use: "무료 원문" },
    ],
    [],
  );
  ...
  return (
    <div className="flex flex-col md:flex-row font-nanum-gothic">
      ...
      <div className={`${mobileSidebar ? "fixed inset-y-0 left-0 z-[7900] flex animate-slide-in-left" : "hidden"} md:relative md:z-auto md:flex w-80 max-w-[85vw] border-r border-white/[0.04] bg-[#0d0f14] flex-col overflow-y-auto flex-shrink-0`}>
        ...
        <div className="md:hidden flex justify-end px-3 pt-2 -mb-2">
          <button
            ...
            aria-label={t("literatureReviewPage.closeAria")}
          >
            ×
          </button>
        </div>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {ACADEMIC_DBS.filter((db) => KOREAN_DB_IDS.has(db.id) && selectedDBs.includes(db.id)).slice(0, 8).map((db) => (
                <a key={db.id} href={buildDbSearchUrl(db, searchQuery)} target="_blank" rel="noopener" className="text-[13px] px-2 py-1 rounded bg-[#6c8cff]/10 text-[#6c8cff] hover:bg-[#6c8cff]/20">
                  {db.name}{t("literatureReviewPage.searchSuffix")}
                </a>
              ))}
            </div>
          )}
      ...
      {/* ── 메인 콘텐츠 영역 ──
           s-renew-15: AI 분석 패널을 겹쳐 배치하기 위해 relative 기준을 만든다. */}
      <div className="relative flex-1 flex flex-col">
        ...
        {/* 탭 콘텐츠 */}
          {/* s-renew-15: AI 분석 컨트롤
               예전에는 본문 흐름 안에 한 줄을 차지해, 탭 바와 검색 영역 사이에
               패널 높이만큼(약 120px) 커다란 빈 띠가 생겼다.
               데스크탑에서는 본문 위 오른쪽에 겹쳐 띄워 그 공백을 없애고,
               오른쪽 끝에서 안쪽으로 더 들여 배치한다.
               좁은 화면(<lg)에서는 겹치면 본문을 가리므로 흐름 안에 그대로 둔다. */}
          <div className="flex justify-end px-4 md:px-6 pt-3 pb-1
                          xl:absolute xl:top-2 xl:right-24 2xl:right-36 xl:z-20 xl:p-0
                          xl:pointer-events-none">
        <div className="rounded-2xl border border-white/[0.08] bg-[#11141d]/80 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.45)] p-1.5 flex flex-col gap-1 w-[188px] xl:pointer-events-auto">
          <div className="flex items-center justify-between px-2 pt-1 pb-0.5">
            <span className="text-[13px] font-semibold tracking-wide text-white/40">{t("literatureReviewPage.aiAnalysis")}</span>
            <span className="text-[12px] text-white/25">{t("literatureReviewPage.selectedCount").replace("{n}", String(selectedResults.length))}</span>
          </div>
          {([
            ...
            >
              ...
              {b.done && (
                <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: b.accent }} title={t("literatureReviewPage.analysisDone")} />
              )}
            </button>
          ))}
        </div>
          </div>
        <div className="overflow-y-auto p-4 md:p-6 xl:pr-[236px] 2xl:pr-[268px]">
          {/* ── 검색 탭 ── */}
          {activeTab === "search" && (
            <div className="max-w-4xl">
              ...
              <div className="mb-5 p-3 rounded-xl border border-white/[0.05] bg-[#13161e]/80">
                <p className="text-[13px] text-white/40 mb-2">{t("litReview.integratedSourcesTitle")}</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {INTEGRATED_SOURCES.map((s) => (
                    <div key={s.name} className="text-[12px] px-2 py-1.5 rounded-lg bg-white/[0.02] border border-white/[0.04]">
                      <span className="text-[#6c8cff] font-medium">{s.name}</span>
                      <span className="text-white/25 block">{s.role}</span>
                    </div>
                  ))}
                </div>
                <p className="text-[12px] text-white/20 mt-2">{t("litReview.rankingNote")}</p>
              </div>
              ...
              <div className="mb-5">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  >
                    <Icon name="database" size={11} /> {t("literatureReviewPage.dbSelect")}
                  </button>
                </div>
              </div>
          ...
          {activeTab === "network" && (
            <NetworkAnalysis
              ...
              onVisualStyleChange={setGraphVisualStyle}
              nodeIcon={graphNodeIcon}
              onNodeIconChange={setGraphNodeIcon}
            />
          )}
          ...
          {activeTab === "analytics" && (
            <LiteratureAnalyticsPanel
              ...
              visualStyle={graphVisualStyle}
              onVisualStyleChange={setGraphVisualStyle}
              chartIcon={graphNodeIcon}
              onChartIconChange={setGraphNodeIcon}
            />
          )}

          {activeTab === "insights" && (
            <LiteratureEngineTools
              papers={selectedPapersForTools}
              locale={locale}
              thesisType={selectedThesisType}
