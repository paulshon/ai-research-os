...
import { getChapters, type Chapter } from "@/lib/research-data";
import { usePagePersistence } from "@/hooks/use-page-persistence";
import { useTranslation } from "@/lib/i18n";
import { localizeChapters, localizeCategories } from "@/lib/i18n/research-labels";
...
import LinkedResultsPanel from "@/components/writing/linked-results-panel";
import PageSaveRegistration from "@/components/save/page-save-bar";
import { usePermissions } from "@/hooks/use-permissions";

const COPILOT_TABS = [
  { key: "chat", icon: "💬", label: "writingPage.tabChat" },
  { key: "verify", icon: "🛡", label: "writingPage.tabVerify" },
  { key: "critique", icon: "📝", label: "writingPage.tabCritique" },
];
...
export default function WritingPage() {
  ...
  const [contents, setContents] = useState<Record<string, string>>({
    "CH.01-0": t("writingPage.introSample"),
  });
  ...
  // Get chapters from structure engine based on thesis type (v16: localized).
  const chapters = useMemo(
    () => localizeChapters(getChapters(thesisType), thesisType, locale, t),
    [thesisType, locale, t]
  );
  // v16: localized categories used in the "Change type" dropdown menu.
  const localizedCategories = useMemo(() => localizeCategories(t), [t]);
  ...
  const currentTypeName = localizedCategories.flatMap((c) => c.types).find((ty) => ty.id === thesisType)?.name ?? t("writingPage.defaultThesisType");

  const askCopilot = async () => {
    if (!prompt.trim()) return;
    const out = await generate({
      systemInstruction: "You are an academic writing copilot. Improve structure, tone, and coherence in Korean.",
      userText: `${t("writingPage.thesisTypeLabel")}: ${currentTypeName}\n${t("writingPage.currentChapterLabel")}: ${currentChapter?.title}\n${t("writingPage.contentLabel")}:\n${content.slice(0, 4000)}\n\n${t("writingPage.requestLabel")}:\n${prompt}`,
      temperature: 0.4,
    });
    setResponse(out);
  };
  ...
  const buildChartFromRows = (rows: string[][]): string | null => {
    ...
    ctx.fillText(t("writingPage.chartTitle"), W / 2, 26);
    ...
    ctx.fillStyle = "#555"; ctx.fillText(t("writingPage.series1Bar"), W / 2 - 62, H - 13);
    if (lineSeries) {
      ctx.strokeStyle = "#e08a3c"; ctx.lineWidth = 2.5;
      ctx.beginPath(); ctx.moveTo(W / 2 + 30, H - 17); ctx.lineTo(W / 2 + 46, H - 17); ctx.stroke();
      ctx.fillStyle = "#555"; ctx.fillText(t("writingPage.series2Line"), W / 2 + 52, H - 13);
    }
    try {
  ...
  const handleImportFile = async (file: File | null) => {
    ...
    setImportMsg(t("writingPage.importing"));
    try {
      if (["jpg", "jpeg", "png", "gif", "webp", "bmp", "svg"].includes(ext)) {
        ...
        setImportMsg(`${t("writingPage.imageInserted")}: ${file.name} · ${t("writingPage.imageInsertedHint")}`);
      } else if (ext === "csv") {
        const rows = parseCSV(await file.text());
        const charted = insertRowsWithChart(rows, file.name);
        setImportMsg(`${t("writingPage.tableWord")}${charted ? ` + ${t("writingPage.chartWord")}` : ""} ${t("writingPage.inserted")}: ${file.name} · ${t("writingPage.clickToResizeHint")}`);
        ...
        setImportMsg(`${t("writingPage.spreadsheetWord")}${charted ? ` + ${t("writingPage.chartWord")}` : ""} ${t("writingPage.inserted")}: ${file.name} · ${t("writingPage.clickToResizeHint")}`);
      } else {
        const text = await file.text();
        editorRef.current?.insertText(text);
        setImportMsg(`${t("writingPage.textInserted")}: ${file.name}`);
      }
    } catch {
      setImportMsg(t("writingPage.importFailed"));
    } finally {
  ...
  const exportPdf = () => {
    ...
    const bodyHtml = content || `<p>(${t("writingPage.noContent")})</p>`;
    const w = window.open("", "_blank");
    if (!w) {
      setImportMsg(t("writingPage.popupBlocked"));
  ...
  const displayProjectName = () =>
    (typeof window !== "undefined" && localStorage.getItem("aros-project-name")) || "Studium R";
    ...
    <div className="flex flex-col font-nanum-gothic h-full">
      ...
      <div className="h-11 bg-[#13161e] border-b border-white/[0.04] flex items-center px-3 md:px-4 gap-2 flex-shrink-0">
        ...
        <div className="hidden md:flex items-center gap-2 min-w-0">
          ...
          <span className="text-[15px] text-white/55 truncate">{sectionTitle || t("writingPage.selectSectionPrompt")}</span>
        </div>
        {/* 모바일 타이틀 */}
        <span className="md:hidden text-[15px] font-medium text-white/70 truncate flex-1 min-w-0">{sectionTitle || t("writingPage.writingTitle")}</span>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <button
            ...
            aria-label={t("writingPage.importImageTable")}
            title={t("writingPage.importImageTableTitle")}
          >
            <Icon name="folderOpen" size={14} />
            <span className="hidden sm:inline text-[14px]">{t("writingPage.import")}</span>
          </button>
          {/* v31: 참고문헌 인용 삽입 버튼 */}
          <button
            ...
            aria-label={t("writingPage.citeReference")}
            title={t("writingPage.insertCitation")}
          >
            <Icon name="citation" size={14} />
            <span className="hidden sm:inline text-[14px]">{t("writingPage.cite")}</span>
          </button>
          {/* v52: 연구방법 절 삽입 버튼 */}
          <button
            onClick={() => setShowMethodPanel(true)}
            className="h-8 px-2 md:px-2.5 flex items-center gap-1 rounded-lg bg-[#3ecfb2]/10 text-[#3ecfb2] border border-[#3ecfb2]/20 hover:bg-[#3ecfb2]/20 transition-colors"
            aria-label={t("writingPage.insertMethodSection")}
            title={t("writingPage.insertMethodEngineResult")}
          >
            <Icon name="method" size={14} />
            <span className="hidden sm:inline text-[14px]">{t("writingPage.method")}</span>
          </button>
          <button
          ...
          <button
            ...
            aria-label={t("writingPage.copy")}
            title={t("writingPage.copy")}
          >
            <Icon name="files" size={14} />
            <span className="hidden md:inline text-[14px]">{t("writingPage.copy")}</span>
          </button>
          <button
            onClick={exportPdf}
            ...
            title={t("writingPage.savePdfTitle")}
          >
            <Icon name="save" size={13} />
            <span className="hidden sm:inline">{t("writingPage.savePdf")}</span>
          </button>
        </div>
      </div>
      {importMsg && (
        ...
        {currentChapter && currentChapter.micro.length > 0 && (
          <div className="relative flex-1 min-w-0">
            <select
              ...
              className="w-full appearance-none pl-3 pr-8 py-2 rounded-lg bg-[#1a1e2a] border border-white/[0.06] text-[15.5px] text-white/80 outline-none focus:border-[#6c8cff]"
            >
              <option value="">{t("writingPage.selectSubsection")}</option>
      ...
      <div className="flex-1 flex min-w-0" style={{minHeight:0}}>
        ...
        {/* Center: Editor */}
        <div className="flex-1 overflow-y-auto p-4 md:p-8 relative min-w-0" style={{WebkitOverflowScrolling:"touch"}}>
          ...
          {copilotCollapsed && (
            ...
            >
              <Icon name="logo" size={14} /> {t("writingPage.expandCopilot")}
            </button>
          )}
          <div className="max-w-[1040px] mx-auto">
            ...
            <RichTextEditor
              ...
              value={content}
              onChange={setContent}
              placeholder={t("writingPage.editorPlaceholder")}
              minHeight={500}
            />
          </div>
        </div>
        ...
        <ResizableRightPanel storageKey="writing" breakpoint="lg" defaultWidth={288} min={240} className={`border-l border-white/[0.04] bg-[#0f1117] overflow-y-auto flex-col flex-shrink-0 ${showMobileCopilot ? "flex w-full absolute inset-0 z-20 lg:relative" : copilotCollapsed ? "hidden" : "hidden lg:flex"}`}>
          <div className="p-3 flex-shrink-0">
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setCopilotCollapsed(true)} className="hidden lg:flex items-center justify-center w-7 h-7 rounded-lg text-white/30 hover:text-white/70 hover:bg-white/[0.05] transition-colors" title={t("writingPage.collapsePanel")} aria-label={t("writingPage.collapsePanel")}><Icon name="chevronRight" size={16} /></button>
            ...
            <p className="text-[13px] text-white/25 mb-3">{t("writingPage.copilotIntro")}</p>
            ...
            <div className="flex gap-1 mb-3">
              {COPILOT_TABS.map((tab, i) => (
                ...
                >
                  <span><Icon name={tab.icon} className="inline-flex align-[-0.125em]" size={15} /></span> {t(tab.label)}
                </button>
              ))}
            </div>
            ...
            <div className="relative mb-3">
              ...
              >
                <span>{t("writingPage.currentSelection")}</span>
                <span className="text-white/30"><Icon name="chevronDown" size={12} /></span>
              </button>
              <p className="text-[13px] text-white/20 mt-1">{t("writingPage.selectSectionOnLeft")}</p>
              ...
              {showTypeMenu && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-[#1a1e2a] border border-white/[0.08] rounded-xl shadow-2xl z-50 max-h-[400px] overflow-y-auto">
                  {localizedCategories.map((cat, ci) => (
                    <div key={ci}>
                      ...
                      {cat.types.map((ty) => {
                        ...
                        <button
                          ...
                          disabled={!allowed}
                          title={allowed ? undefined : t("writingPage.upperTierOnly")}
            ...
            <div className="space-y-3 text-[15px] text-white/35">
              {copilotTab === 0 && <p>{t("writingPage.copilotTabChatDesc")}</p>}
              {copilotTab === 1 && <p>{t("writingPage.copilotTabVerifyDesc")}</p>}
              {copilotTab === 2 && <p>{t("writingPage.copilotTabCritiqueDesc")}</p>}
            </div>
            <input
              value={prompt}
              ...
              placeholder={t("writingPage.askAiPlaceholder")}
            />
            <button onClick={askCopilot} disabled={loading} className="w-full mt-2 py-2 rounded-lg bg-[#a78bfa]/20 text-[#a78bfa] text-[15px] disabled:opacity-40">
              {loading ? "..." : t("writingPage.runCopilot")}
            </button>
            {response && (
              <div className="mt-3">
                ...
                >
                  <Icon name="download" size={12} /> {t("writingPage.insertIntoEditor")}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="mt-auto p-3 border-t border-white/[0.04]">
            ...
            >
              ← {t("writingPage.backToBasicMode")}
            </Link>
          </div>
        </ResizableRightPanel>
      </div>

      {/* v20: 연구설계·문헌연구 연동 결과 (하단 리사이즈·아코디언) */}
      <LinkedResultsPanel
        onInsert={(text) => {
          editorRef.current?.insertText(text);
        }}
