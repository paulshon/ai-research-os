  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 font-nanum-gothic">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-[24px] font-bold font-nanum-myeongjo mb-8">
          <Icon name="⚙" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("settings.title")}
        </h1>

        {/* v26: 테마(화면 모드) 설정 */}
        <section className="mb-8 p-6 rounded-[18px] bg-[#13161e] border border-white/[0.04]">
          <h2 className="text-[16px] font-semibold mb-1">{t("settings.themeTitle")}</h2>
          <p className="text-[12px] text-white/25 mb-5">{t("settings.themeDesc")}</p>
          <ThemeSwitcher />
        </section>

        <section className="mb-8 p-6 rounded-[18px] bg-[#13161e] border border-white/[0.04]">
          <h2 className="text-[16px] font-semibold mb-1">{t("settings.aiTitle")}</h2>
          <p className="text-[12px] text-white/25 mb-5">{t("settings.aiDesc")}</p>

          <div className="space-y-5">
            {/* API Keys by Provider */}
            <div>
              <label className="block text-[12px] text-white/30 mb-3">프로바이더별 API Key</label>
              <div className="space-y-3">
                {uniqueProviders.map(provider => {
                  const providerModel = MODELS.find(m => m.provider === provider);
                  return (
                    <div key={provider} className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2">
                      <div className="flex items-center justify-between sm:w-28 sm:flex-shrink-0">
                        <label className="text-[11px] text-white/40">{provider}</label>
                        {providerModel && (
                          <a href={providerModel.apiUrl} target="_blank" rel="noopener" className="text-[10px] text-[#6c8cff] hover:underline whitespace-nowrap sm:hidden">
                            키 발급
                          </a>
                        )}
                      </div>
                      <input
                        ...
                        className="w-full sm:flex-1 min-w-0 px-3 py-2 rounded-[8px] bg-[#1a1e2a] border border-white/[0.06] text-[12px] text-white font-['JetBrains_Mono',monospace] focus:border-[#6c8cff] focus:outline-none"
                        placeholder={`${provider} API Key`}
                      />
                      {providerModel && (
                        <a href={providerModel.apiUrl} target="_blank" rel="noopener" className="hidden sm:inline text-[10px] text-[#6c8cff] hover:underline whitespace-nowrap">
                          키 발급
                        </a>
                      )}
                    </div>
              ...
              <p className="text-[10px] text-white/20 mt-2">현재 선택된 모델의 프로바이더 키가 자동으로 사용됩니다.</p>
            </div>

            {/* 모델 선택 */}
            <div>
              <label className="block text-[12px] text-white/30 mb-2">{t("settings.model")}</label>
              <select
                value={model}
                onChange={e => handleModelChange(e.target.value)}
                className="w-full px-4 py-3 rounded-[10px] bg-[#1a1e2a] border border-white/[0.06] text-[14px] text-white/60 focus:border-[#6c8cff] focus:outline-none"
              >
              ...
              <div className={`mt-3 p-3 rounded-lg border ${
                ...
                <p className="text-[11px] text-white/30">{selectedModelInfo.desc}</p>
                <a
                  href={selectedModelInfo.apiUrl}
                  target="_blank"
                  rel="noopener"
                  className="text-[10px] text-[#6c8cff] hover:underline mt-1 inline-flex items-center gap-1"
                >
              ...
              {selectedModelInfo.tier === "free" && (
                <div className="mt-3 p-3 rounded-lg border border-[#e8b84b]/20 bg-[#e8b84b]/[0.04]">
                  <p className="text-[11px] text-[#e8b84b] font-medium mb-1"><Icon name="⚠" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("settings.freeNoticeTitle")}</p>
                  <p className="text-[10px] text-white/30 leading-relaxed">{t("settings.freeNoticeDesc")}</p>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={save}
                className="px-6 py-3 bg-[#4a6cf7] hover:bg-[#5d7dff] text-white rounded-[12px] text-[14px] font-medium transition-all"
              >
                {saved ? `${t("settings.saved")}` : t("settings.saveBtn")}
              </button>
              <button
                onClick={() => router.back()}
                className="px-6 py-3 bg-white/[0.05] hover:bg-white/[0.08] text-white/50 hover:text-white/70 border border-white/[0.06] rounded-[12px] text-[14px] font-medium transition-all"
              >
                {t("settings.close")}
              </button>
            </div>
        ...
        {/* ── API 키 발급 링크 모음 ── */}
        <section className="mb-8 p-6 rounded-[18px] bg-[#13161e] border border-white/[0.04]">
          <h2 className="text-[16px] font-semibold mb-1">{t("settings.apiLinksTitle")}</h2>
          <p className="text-[12px] text-white/25 mb-4">{t("settings.apiLinksDesc")}</p>
            ...
            {[...new Map(MODELS.map(m => [m.apiLabel, m])).values()].map(m => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] text-white/60 font-medium group-hover:text-white/80 truncate">
                    {m.apiLabel}
                  </p>
                  <p className="text-[10px] text-white/20 truncate">{m.provider}</p>
                </div>
                <span className="text-[10px] text-white/15 group-hover:text-[#6c8cff]">→</span>
              </a>
            ))}
          </div>
        </section>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center" style={{ background: "rgba(0,0,0,0.6)" }}>
          <div className="w-[420px] p-6 rounded-[18px] bg-[#1a1e2a] border border-[#ff7066]/20 shadow-2xl">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-[16px] font-semibold text-white">{t("settings.paidWarningTitle")}</h3>
            </div>
            <p className="text-[13px] text-white/60 leading-relaxed mb-2">
              {t("settings.paidWarningMsg1")}
            </p>
            <p className="text-[13px] text-[#ff7066] font-medium leading-relaxed mb-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 bg-[#ff7066] hover:bg-[#ff8a82] text-white rounded-[12px] text-[14px] font-medium transition-all"
              >
                {t("settings.paidConfirm")}
              </button>
              <button
                onClick={cancelPaidModel}
                className="flex-1 py-3 bg-white/[0.05] hover:bg-white/[0.08] text-white/50 border border-white/[0.06] rounded-[12px] text-[14px] font-medium transition-all"
              >
