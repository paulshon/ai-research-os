...
import { koDash3 } from "@/lib/i18n/locales/ko-dash3";
import { enDash3 } from "@/lib/i18n/locales/en-dash3";
import { zhDash3 } from "@/lib/i18n/locales/zh-dash3";
...
interface ChecklistItem { label: string; done: boolean; score: number; key?: string; }
...
const CHECKLIST_LABELS_BY_KEY: Record<string, string[]> = (() => {
  const keys = [
    "schedulePage.checklistFormat",
    "schedulePage.checklistReferences",
    "schedulePage.checklistAbstract",
    "schedulePage.checklistCaptions",
    "schedulePage.checklistPlagiarism",
    "schedulePage.checklistSpelling",
    "schedulePage.checklistAiDetection",
    "schedulePage.checklistCoverLetter",
  ];
  const pick = (dict: Record<string, unknown>, k: string) => {
    const parts = k.split(".");
  ...
  for (const k of keys) {
    out[k] = [
      pick(koDash3 as unknown as Record<string, unknown>, k),
      pick(enDash3 as unknown as Record<string, unknown>, k),
      pick(zhDash3 as unknown as Record<string, unknown>, k),
    ].filter(Boolean);
  }
  return out;
})();

const DEFAULT_CHECKLIST_KEYS: string[] = [
  "schedulePage.checklistFormat",
  "schedulePage.checklistReferences",
  "schedulePage.checklistAbstract",
  "schedulePage.checklistCaptions",
...
export default function SchedulePage() {
  ...
  // ── 제출 체크리스트 (구 제출 준비) ──
  // v16: store `key` alongside label so language changes re-translate default items
  const defaultChecklist = useMemo<ChecklistItem[]>(
    () => DEFAULT_CHECKLIST_KEYS.map((key) => ({ key, label: t(key), done: false, score: 0 })),
    [t]
  );
  const [checklist, setChecklist] = useState<ChecklistItem[]>(defaultChecklist);
  const [newItem, setNewItem] = useState("");

  // v16: Localized view of checklist — default items translate from key; user-added items keep their label.
  const localizedChecklist = useMemo<ChecklistItem[]>(
    () =>
      checklist.map((item) => {
        // If the item has a known key, always translate via t() so locale switches propagate.
        if (item.key && DEFAULT_CHECKLIST_KEYS.includes(item.key)) {
          return { ...item, label: t(item.key) };
        }
        // Legacy items (saved from earlier versions) may match by label — auto-heal by attaching a key.
        const matchIdx = DEFAULT_CHECKLIST_KEYS.findIndex((k) => {
          const labels = CHECKLIST_LABELS_BY_KEY[k] ?? [];
          return labels.includes(item.label) || item.label === t(k);
        });
        if (matchIdx >= 0) {
          const k = DEFAULT_CHECKLIST_KEYS[matchIdx];
          return { ...item, key: k, label: t(k) };
        }
        return item;
      }),
    [checklist, t]
  ...
  // v16: identify items by index instead of label — labels change on locale switch, index is stable.
  ...
  const exportContent = (format: "pdf" | "word" | "latex") => {
    const writingDraft = localStorage.getItem("aros-page-writing") || t("schedulePage.noBodyData");
    const payload = format === "latex" ? `\\section{${t("schedulePage.thesisDraft")}}\n${writingDraft}` : writingDraft;
    ...
    <div className="p-4 md:p-5 lg:px-6 lg:py-6 font-nanum-gothic">
      ...
      <div className="max-w-[1680px] mx-auto flex flex-col xl:flex-row gap-6 items-start">
        ...
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-[17px] font-extrabold font-nanum-myeongjo"><Icon name="calendar" className="inline-flex align-[-0.125em] mr-1" size={16} /> {t("schedulePage.paperSchedule")}</h2>
        ...
        <p className="text-[16px] text-white/35 mb-6">{t("schedulePage.subtitle")}</p>
              ...
              {phases.map((p) => (
                <div key={p.id} className="flex items-center gap-2 p-2 rounded-lg bg-[#0d0f14] border border-white/[0.04]">
                  ...
                  <input type="date" value={phaseSchedules[p.id]?.start || ""} onChange={e => updatePhaseSchedule(p.id, "start", e.target.value)}
                    className="flex-1 px-2 py-1 rounded-md bg-[#1a1e2a] border border-white/[0.04] text-[13px] text-white/50 focus:outline-none min-w-0" />
        ...
        <div className="flex gap-2 mb-4">
          ...
          <input value={newTaskTitle} onChange={(e) => setNewTaskTitle(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addTask()}
            className="flex-1 px-3 py-2 rounded-lg bg-[#1a1e2a] border border-white/[0.06] text-[16px] text-white focus:border-[#6c8cff] focus:outline-none min-w-0"
        ...
        <div className="mt-10 mb-3 flex items-center gap-2">
          ...
          <h2 className="text-[19px] font-bold font-nanum-myeongjo">{t("schedulePage.finalCheckTitle")}</h2>
          <span className="text-[14px] text-white/30">{t("schedulePage.finalChecklistDesc")}</span>
        </div>
        ...
        <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04] mb-6 flex items-center gap-4">
          <div className="text-center min-w-[80px]">
            ...
            <p className="text-[14px] text-white/30">{t("schedulePage.readiness")}</p>
          </div>
          <div className="flex-1 h-3 rounded-full bg-white/[0.06] min-w-0">
        ...
        <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04] mb-6">
          <p className="text-[16px] font-semibold text-[#e8eaf0] mb-4">{t("schedulePage.finalChecklist")}</p>
          <div className="space-y-2">
            {localizedChecklist.map((item, idx) => (
              <button key={item.key || `custom-${idx}-${item.label}`} onClick={() => toggleItem(idx)} className="w-full flex items-center gap-3 p-3 rounded-[10px] bg-[#1a1e2a] border border-white/[0.04] text-left">
          ...
          <div className="mt-3 flex gap-2">
            <input value={newItem} onChange={(e) => setNewItem(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addItem()}
              className="flex-1 px-3 py-2 rounded-lg bg-[#1a1e2a] border border-white/[0.06] text-[15px] text-white focus:outline-none focus:border-[#e8b84b] min-w-0"
              placeholder={t("schedulePage.addChecklistItemPlaceholder")} />
            <button onClick={addItem} className="px-3 py-2 rounded-lg bg-[#e8b84b]/20 text-[#e8b84b] text-[15px]">{t("schedulePage.addBtn")}</button>
          </div>
        </div>

        <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04]">
          <p className="text-[16px] font-semibold text-[#e8eaf0] mb-4">{t("schedulePage.exportManuscript")}</p>
          ...
          <p className="text-[14px] text-white/25 mt-3">{t("schedulePage.exportHint")}</p>
        </div>
        </div>{/* /flex-1 main */}
        ...
        <aside className="w-full xl:w-[320px] flex-shrink-0 xl:sticky xl:top-4 space-y-3">
          <div className="p-4 rounded-2xl bg-[#13161e] border border-white/[0.05]">
            <p className="text-[13px] font-semibold text-white/70 mb-3">{t("schedulePage.overallStatus")}</p>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[12px] text-white/55">{t("schedulePage.completedTasks")}</p>
                ...
                <p className="text-[11px] text-white/30 mt-0.5">{t("schedulePage.readiness")} {readiness}%</p>
              </div>
            </div>
            <div className="space-y-2">
              {phases.map((p) => {
          ...
          <div className="p-4 rounded-2xl bg-[#13161e] border border-white/[0.05]">
            <p className="text-[13px] font-semibold text-white/70 mb-2.5 flex items-center gap-1.5"><Icon name="checklist" size={14} /> {t("schedulePage.nextTasks")}</p>
            {localizedTasks.filter((tk) => !tk.done).length === 0 ? (
              <p className="text-[12px] text-white/30 py-1">{t("schedulePage.allTasksDone")} 🎉</p>
            ) : (
                ...
                {localizedTasks.filter((tk) => !tk.done).slice(0, 6).map((tk) => {
                  ...
                  return (
                    <button key={tk.id} onClick={() => toggleTask(tk.id)}
                      ...
                      <span className="flex-1 text-[12px] text-white/60 truncate min-w-0">{tk.title}</span>
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-br from-[#6c8cff]/[0.08] to-[#a78bfa]/[0.05] border border-[#6c8cff]/15">
            <p className="text-[13px] font-semibold text-[#8aa4ff] mb-2 flex items-center gap-1.5"><Icon name="logo" size={14} /> {t("schedulePage.aiRecommendation")}</p>
            <ul className="space-y-1.5 text-[12px] text-white/55">
              <li className="flex items-start gap-1.5"><span className="text-[#6c8cff] mt-0.5">→</span><span>{t("schedulePage.aiTip1")}</span></li>
              <li className="flex items-start gap-1.5"><span className="text-[#6c8cff] mt-0.5">→</span><span>{t("schedulePage.aiTip2")}</span></li>
              <li className="flex items-start gap-1.5"><span className="text-[#6c8cff] mt-0.5">→</span><span>{t("schedulePage.aiTip3")}</span></li>
            </ul>
          </div>
        </aside>
      </div>
