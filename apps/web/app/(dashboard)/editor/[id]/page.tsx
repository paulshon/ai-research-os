    ...
    <div className="flex h-full font-nanum-gothic">
      ...
      {showSaveDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#1a1e2a] border border-white/10 rounded-2xl p-6 w-[360px] shadow-2xl">
            <h3 className="text-[15px] font-semibold mb-1">{t("editor.saveDialogTitle")}</h3>
            <p className="text-[11px] text-white/35 mb-4">{t("editor.saveDialogDesc")}</p>
            <div className="mb-3">
              <label className="text-[11px] text-white/40 block mb-1">{t("editor.fileName")}</label>
            ...
            <p className="text-[10px] text-white/20 mb-4">{t("editor.saveHint")}</p>
            <div className="flex gap-2">
              <button onClick={saveToLocal} className="flex-1 py-2 bg-[#4a6cf7] text-white rounded-xl text-[13px] font-medium hover:bg-[#5d7dff] transition-colors">
                {t("save.save")}
              </button>
              <button onClick={() => setShowSaveDialog(false)} className="px-4 py-2 bg-white/[0.04] text-white/40 rounded-xl text-[13px] hover:bg-white/[0.08] transition-colors">
                {t("common.cancel")}
              </button>
            </div>
          </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.08]">
              <h3 className="text-[13px] font-semibold">{t("editor.selectSection")}</h3>
              <button
                onClick={() => setShowSectionPicker(false)}
            ...
            <div className="overflow-y-auto p-3">
              {chapters.map((ch, ci) => (
                <div key={ci} className="mb-2">
                  <p className="text-[11px] font-semibold text-white/40 px-2 py-1" style={{ color: ch.color }}>
                  ...
                  {ch.micro.map((m, mi) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 rounded-md text-[11px] transition-colors ${
                          isActive
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="h-10 border-b border-white/[0.04] flex items-center px-4 gap-2">
          <div className="flex-1 min-w-0">
            ...
            <button
              ...
              className="md:hidden w-full text-left text-[13px] font-medium truncate"
          ...
          <button onClick={generateAI} disabled={loading || !selectedSection} className="px-3 py-1 rounded-md text-[11px] bg-[#4a6cf7]/10 text-[#6c8cff] border border-[#4a6cf7]/20 disabled:opacity-30">
            {loading ? t("editor.generating") : t("editor.aiGenerate")}
          </button>
          <button onClick={() => navigator.clipboard.writeText(content)} className="px-3 py-1 rounded-md text-[11px] bg-white/[0.04] border border-white/[0.06] text-white/40">
            {t("editor.copy")}
          </button>
          <button onClick={save} className="px-3 py-1 rounded-md text-[11px] bg-[#4a6cf7] text-white">
        ...
        <textarea
          ...
          className="flex-1 px-6 py-4 bg-transparent text-[14px] text-white/80 leading-[1.9] focus:outline-none resize-none font-nanum-myeongjo"
      ...
      <aside className="hidden lg:block w-80 border-l border-white/[0.04] bg-[#0d0f14] overflow-hidden">
        <div className="h-full flex flex-col">
          <div className="p-4 border-b border-white/[0.04]">
            <h3 className="text-[13px] font-semibold text-white/90">AI Copilot</h3>
          ...
          <div className="p-4 flex-1 overflow-y-auto">
            <div className="grid grid-cols-2 gap-2">
              <Link
                href="/chat"
                className="px-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.06] text-[12px] text-white/70 hover:bg-white/[0.07] hover:text-white transition-colors flex items-center gap-2"
              >
            ...
            <div className="mt-4 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
              <p className="text-[10px] text-white/30">현재 선택</p>
              {currentChapter && currentMicro && selectedSection ? (
                ...
                <p className="text-[12px] text-white/40 mt-1">{t("editor.selectLeft")}</p>
              )}
            </div>
          </div>
        </div>
