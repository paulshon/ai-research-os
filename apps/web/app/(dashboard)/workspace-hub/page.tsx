  ...
  return (
    <div className="p-4 md:p-5 lg:px-6 lg:py-6 font-nanum-gothic">
      <div className="max-w-[1680px] mx-auto">
        <p className="text-[14px] text-white/20 font-mono mb-1">Workspace — AI Research OS</p>
        <h1 className="font-nanum-myeongjo text-[25px] font-bold text-[#e8eaf0] mb-1">워크스페이스</h1>
        <p className="text-[16px] text-white/35 mb-6">프로젝트 관리, 팀 협업, 연구 기억</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          {PROJECTS.map((p) => (
            ...
            >
              <p className="text-[17px] font-semibold text-[#e8eaf0] mb-1">{p.title}</p>
              <span className="text-[13px] px-2 py-0.5 rounded-full" style={{ background: p.color + "15", color: p.color }}>{p.status}</span>
          ...
          <button className="p-4 rounded-[14px] border border-dashed border-white/[0.08] hover:border-white/[0.15] text-white/20 hover:text-white/40 transition-all text-center">
            ...
            <p className="text-[15px] mt-1">새 프로젝트</p>
          </button>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04]">
            <p className="text-[16px] font-semibold text-[#e8eaf0] mb-3"><Icon name="📊" className="inline-flex align-[-0.125em] mr-1" size={15} />Knowledge Graph</p>
            <div className="h-[200px] flex items-center justify-center text-white/15 text-[16px]">
              논문-개념-인용 네트워크 시각화 (준비 중)
            </div>
          </div>
          <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04]">
            <p className="text-[16px] font-semibold text-[#e8eaf0] mb-3"><Icon name="💡" className="inline-flex align-[-0.125em] mr-1" size={15} />Research Memory</p>
            <div className="h-[200px] flex items-center justify-center text-white/15 text-[16px]">
              아이디어 진화 기록 (준비 중)
            </div>
          </div>
