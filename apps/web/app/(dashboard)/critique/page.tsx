...
import { saveCritiquePdfBase64, loadCritiquePdfBase64, setCritiquePdfMemory } from "@/lib/critique-pdf-store";
import {
  processDocument,
  toFormattedText,
  isSupportedFile,
...
async function extractPdf(
  ...
  /* ove-4: 300~500p 에서도 메인 스레드를 양보하며 추출 */
  const yieldEvery = totalPages > 200 ? 4 : totalPages > 80 ? 6 : 10;
...
export default function CritiquePage() {
  ...
  const [pdfBase64, setPdfBase64State] = useState<string | null>(null);
  const setPdfBase64 = useCallback((b64: string | null) => {
    setPdfBase64State(b64);
    setCritiquePdfMemory(b64);
    void saveCritiquePdfBase64(b64);
  }, []);
  ...
  /* ove-4: 썸네일 가상 윈도우 — 300~500p 에서도 DOM/렌더 부하를 제한 */
  const [thumbWindow, setThumbWindow] = useState({ start: 0, end: 24 });
  ...
  const applyDraft = useCallback((raw: unknown) => {
    ...
    if (d.pdfBase64) setPdfBase64(d.pdfBase64);
      ...
      /* LS 에 PDF 가 없으면 IndexedDB 에서 복구 */
      void loadCritiquePdfBase64().then((b64) => {
        if (b64) {
          setPdfBase64(b64);
          if (!hasThumbnails && pages.length > 0) setPendingPdfBase64(b64);
        }
      });
    }
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let workFile = file;
    if (isHwpFile(file)) {
      const hwpTok = aiProgress.begin(t("critiquePage.hwpConverting"));
      const conv = await convertHwpToPdf(file, (m) => aiProgress.setLabel(m));
      aiProgress.end(hwpTok);
      if (conv.pdf) {
        workFile = conv.pdf;
        setUodeMeta(t("critiquePage.hwpConvertedMsg"));
      } else if (conv.reason === "failed" && conv.message) {
        setUodeMeta(`${t("critiquePage.hwpConverterUnavailable")} (${conv.message})`);
      }
    }
    ...
    if (
      ...
      try {
        ...
        setUodeMeta(
          `${formatLabel(doc.format)} · ${doc.charCount.toLocaleString()}${t("critiquePage.charsUnit")} · ` +
            `${doc.pageCount}p · ${t("critiquePage.confidenceLabel")} ${doc.confidence}%${note}`
        );
        ...
        setDocText(
          `[${workFile.name}]\n${t("critiquePage.docExtractFailed")}: ${
            err?.message || t("critiquePage.unknownError")
          }\n${t("critiquePage.enterOrPasteText")}`
        );
        setLoadingText(true);
      } finally {
        setRenderProgress(null);
      ...
      setDocText(
        `[${workFile.name}]\n${t("critiquePage.unsupportedFormat")}`
      );
      setLoadingText(true);
    }
    e.target.value = "";
  ...
  const addPageCritique = (pageNum: number) => {
    ...
    const newCard: CritiqueCard = {
      id: Date.now().toString(), num, type: activeType,
      text: `[${pageNum}${t("critiquePage.pageWholeSuffix")}]`, note: "", editNote: "",
      status: "open", pageNum,
    };
    setCards(prev => [...prev, newCard]);
    setEditingId(newCard.id);
  ...
  const aiAnalyze = async () => {
    ...
    if (isPdf && pdfPages.length > 0) {
      analysisText = pdfPages
        .map((p) => `[${t("critiquePage.pagePrefix")} ${p.pageNum}]\n${p.text}`)
        .join("\n\n")
        .trim();
    }
  ...
  return (
    <div className="flex flex-col lg:flex-row font-nanum-gothic lg:h-[calc(100dvh-2.5rem)] lg:overflow-hidden" style={{ minHeight: "0" }}>
      ...
      <div className="flex flex-1 lg:h-full lg:overflow-hidden min-w-0" style={{ minHeight: "0" }}>
      ...
      <div className={`${mobilePane === "structure" ? "flex" : "hidden"} lg:flex w-full lg:w-48 border-r border-white/[0.04] bg-[#0d0f14] flex-col flex-shrink-0 lg:h-full lg:overflow-hidden`}>
        ...
        <div
          ref={thumbListRef}
          className="flex-1 overflow-y-auto px-2 pb-3 min-w-0"
          onScroll={(e) => {
            const el = e.currentTarget;
            const itemH = 88;
            const start = Math.max(0, Math.floor(el.scrollTop / itemH) - 4);
            const visible = Math.ceil(el.clientHeight / itemH) + 8;
            setThumbWindow({ start, end: start + visible });
          }}
        >
          {pdfPages.length > 0 ? (
            <>
              ...
              <div className="space-y-1.5" style={{ minHeight: pdfPages.length * 88 }}>
                {pdfPages.map((pg, idx) => {
                  const inWindow = idx >= thumbWindow.start && idx < thumbWindow.end;
                  if (!inWindow) {
                    return <div key={pg.pageNum} style={{ height: 84 }} aria-hidden />;
                  }
                    ...
                    <div key={pg.pageNum}
                      ...
                      {pdfDoc ? (
                        <LazyPdfPage
                          doc={pdfDoc}
                          pageNum={pg.pageNum}
                          scale={pdfPages.length > 200 ? 0.28 : 0.4}
                          quality={pdfPages.length > 200 ? 0.42 : 0.5}
                          thumb
                          imgClassName="w-full"
                        />
                      ) : (
                ...
                {docText.split(/\n{2,}/).filter(p => p.trim()).slice(0, 20).map((p, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-1 px-1 py-0.5 rounded hover:bg-white/[0.02]">
                      ...
                      <span className="text-[13px] text-white/25 truncate flex-1 min-w-0">{p.slice(0, 30)}</span>
      ...
      <div className={`${mobilePane === "viewer" ? "flex" : "hidden"} lg:flex flex-1 flex-col border-r border-white/[0.04] lg:h-full lg:overflow-hidden`} style={{ minWidth: 0 }}>
        ...
        {/* ── 뷰어 본문 ── */}
        <div ref={viewerRef} className="flex-1 overflow-y-auto min-w-0" style={{ fontSize }}>
          {/* 렌더링 진행 중 */}
          {renderProgress && (
                ...
                return (
                  <div key={pg.pageNum} id={`page-${pg.pageNum}`}>
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="flex-1 h-px bg-white/[0.04] min-w-0" />
                    </div>
          ...
          <div className="fixed z-50 bg-[#1e2230] border border-white/10 rounded-xl shadow-2xl p-3 min-w-[210px]"
            ...
            <div className="flex gap-2">
              <button onClick={createCritiqueFromSelection}
                className="flex-1 py-1.5 bg-[#4a6cf7] text-white rounded-lg text-[14px] font-medium min-w-0">
        ...
        {selPopup && selText && isMobile && (
          <div className="lg:hidden fixed inset-x-0 z-[8200] bottom-[calc(3.5rem_+_env(safe-area-inset-bottom,0px))] md:bottom-0 bg-[#1e2230] border-t border-white/10 rounded-t-2xl shadow-2xl px-4 pt-3 pb-4 md:pb-[max(16px,env(safe-area-inset-bottom))] animate-fade-in">
            ...
            <div className="flex gap-2">
              <button onClick={createCritiqueFromSelection}
                className="flex-1 py-3 bg-[#4a6cf7] text-white rounded-xl text-[16px] font-semibold active:scale-[0.98] transition-transform min-w-0">
                {t("critique.createCritique").replace("{n}", String(cards.length + 1))}
              </button>
              <button onClick={() => { setSelPopup(null); window.getSelection()?.removeAllRanges(); }}
      ...
      <ResizableRightPanel storageKey="critique" breakpoint="lg" defaultWidth={280} min={240} className={`${mobilePane === "critique" ? "flex" : "hidden"} lg:flex w-full flex-col border-l border-white/[0.04] lg:h-full lg:overflow-hidden`}>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2 min-w-0" style={{WebkitOverflowScrolling:"touch"}}>
              ...
              return (
                <div key={c.id} id={`card-${c.id}`}
                    ...
                    {editingId === c.id ? (
                      <>
                        ...
                        <div className="flex gap-1.5 mt-1.5">
                          <button onClick={() => saveNote(c.id)} className="flex-1 py-1 bg-[#4a6cf7] text-white rounded-md text-[13px] font-medium min-w-0">{t("common.ok")}</button>
                  ...
                  <div className="flex gap-1 px-3 pb-2.5">
                    <button onClick={() => updateStatus(c.id, "in-revision")} className="flex-1 py-0.5 text-[12px] rounded bg-[#e8b84b]/10 text-[#e8b84b] hover:bg-[#e8b84b]/20 min-w-0">{t("critique.requestRevision")}</button>
                    <button onClick={() => updateStatus(c.id, "resubmitted")} className="flex-1 py-0.5 text-[12px] rounded bg-[#6c8cff]/10 text-[#6c8cff] hover:bg-[#6c8cff]/20 min-w-0">{t("critique.resubmit")}</button>
                    <button onClick={() => updateStatus(c.id, "approved")} className="flex-1 py-0.5 text-[12px] rounded bg-[#5ebd7c]/10 text-[#5ebd7c] hover:bg-[#5ebd7c]/20 min-w-0">{t("critique.approve")}</button>
                  </div>
                </div>
              );
            })
