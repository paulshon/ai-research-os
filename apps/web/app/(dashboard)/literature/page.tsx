...
export default function LiteraturePage() {
  ...
  const syncGapToWriting = useCallback(
    (gap: string, refs: ReferenceItem[]) => {
      const papers: LinkedPaper[] = refs.map((r) => ({
        title: r.title,
        authors: r.authors,
        year: r.year,
        journal: r.journal,
      ...
      if (!gap.trim() && papers.length === 0) return;
      upsertLinkedResult({
        key: "lit-gap-search",
        group: "literature",
        title: t("writingPage.linked.litGapSearch"),
        body: gap,
        papers,
      });
    },
  ...
  // 논문작성 연동: 갭분석·논문리스트 동기화
  useEffect(() => {
    if (gapResult || references.length > 0) {
      syncGapToWriting(gapResult, references);
    }
  }, [gapResult, references, syncGapToWriting]);
  ...
  const runGapAnalysis = async () => {
    ...
    const langInstruction = locale === "ko" ? t("literaturePage.respondInKorean") : locale === "zh" ? t("literaturePage.respondInChinese") : "";
    ...
    setGapResult(result);
    setTab("gap");
    syncGapToWriting(result, references);
  };
    ...
    <div className="flex flex-col font-nanum-gothic">
        ...
        {LIT_TABS.map((lt) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[15px] whitespace-nowrap transition-all flex items-center gap-1.5 ${
              tab === lt.id ? "bg-[#1e2230] text-white" : "text-white/30 hover:text-white/50 hover:bg-white/[0.03]"
            }`}
          >
            <Icon name={lt.icon} className="inline-flex align-[-0.125em]" size={15} /> {t(lt.label)}
          </button>
        ))}
      </div>
      ...
      <div className="overflow-y-auto p-4 md:p-5 lg:px-6 lg:py-6">
        <div className="max-w-[1680px] mx-auto">
          {tab === "search" && (
            <div>
              ...
              <div className="flex items-center gap-2 mb-5">
                ...
                >
                  <Icon name="flag" size={14} className="inline-flex align-[-0.125em] mr-1" /> {t("literaturePage.domestic")}
                </button>
                <button
                ...
                <div className="ml-auto">
                  <span className="text-[13px] text-white/20">{t("literaturePage.collectedPapers")}: {references.length}{t("literaturePage.papersUnit")}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
                ...
                <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04] min-h-[300px]">
                  <p className="text-[16px] text-white/25 mb-3">{t("literaturePage.searchResultsRefList")}</p>
                  {references.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="w-full text-[14px]">
                        <thead>
                          <tr className="text-white/30 border-b border-white/[0.06]">
                            <th className="text-left py-2 px-2 w-12">{t("literaturePage.year")}</th>
                            <th className="text-left py-2 px-2">{t("literaturePage.titleCol")}</th>
                            <th className="text-left py-2 px-2 w-28">{t("literaturePage.authors")}</th>
                            <th className="text-left py-2 px-2 w-28">{t("literaturePage.publisherJournal")}</th>
                            <th className="text-left py-2 px-2 w-20">DOI</th>
                            <th className="text-left py-2 px-2 w-20">{t("literaturePage.link")}</th>
                    ...
                    <div className="flex items-center justify-center h-[200px] text-white/15 text-[16px]">
                      {t("literaturePage.enterKeywordHint")}
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04]">
                    ...
                    <p className="text-[15px] text-white/35 mb-3">{t("literaturePage.copilotDesc")}</p>
                    <Link href="/literature-review" className="text-[14px] text-[#3ecfb2] hover:underline block mb-2">→ {t("literaturePage.detailedLitReviewEngine")}</Link>
                    <button
                    ...
                    >
                      {aiLoading ? t("literaturePage.analyzing") : `${t("literaturePage.runGapAnalysis")} (${references.length}${t("literaturePage.papersUnit")})`}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5">
                <p className="text-[16px] font-semibold text-[#e8eaf0] mb-3 flex items-center gap-2"><Icon name="chart" size={17} /> {t("literaturePage.analysisDashboard")}</p>
                {(() => {
                  ...
                  const stats = [
                    { label: t("literaturePage.collectedPapers"), value: refs.length, color: "#3ecfb2" },
                    { label: t("literaturePage.yearRange"), value: years.length ? `${minY}–${maxY}` : "—", color: "#6c8cff" },
                    { label: t("literaturePage.uniqueJournals"), value: journals.size || "—", color: "#a78bfa" },
                    { label: t("literaturePage.uniqueAuthors"), value: authors.size || "—", color: "#e8b84b" },
                  ];
                  return (
                    <>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-3">
                        <div className="p-4 rounded-[14px] bg-[#13161e] border border-white/[0.05]">
                          <p className="text-[14px] font-semibold text-white/70 mb-3">{t("literaturePage.yearlyDistribution")}</p>
                          {yearBars.length > 0 ? (
                            ...
                            <div className="h-32 flex items-center justify-center text-white/15 text-[14px]">{t("literaturePage.yearDistributionAfterSearch")}</div>
                          )}
                        </div>
                        <div className="p-4 rounded-[14px] bg-[#13161e] border border-white/[0.05]">
                          <p className="text-[14px] font-semibold text-white/70 mb-3">{t("literaturePage.topJournals")}</p>
                          {journals.size > 0 ? (
                            ...
                            <div className="h-32 flex items-center justify-center text-white/15 text-[14px]">{t("literaturePage.journalListAfterSearch")}</div>
                          )}
                          <Link href="/literature-review" className="inline-flex items-center gap-1 mt-3 text-[13px] text-[#3ecfb2] hover:underline">
                            {t("literaturePage.detailedNetworkAnalysis")} →
                          </Link>
                        </div>
                      </div>
                    </>
          ...
          {tab === "gap" && (
            <div>
              <h2 className="text-[21px] font-bold font-nanum-myeongjo mb-4"><Icon name="🔬" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("literaturePage.gapAnalysisResultTitle")}</h2>
              {gapResult ? (
                <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04]">
                  <div className="text-[16px] text-white/70 leading-[1.8] whitespace-pre-wrap mb-4">{gapResult}</div>
                  <div className="border-t border-white/[0.06] pt-4">
                    <p className="text-[15px] text-white/30 mb-2">{t("literaturePage.addMemo")}</p>
                    <textarea
                      value={userNotes}
                      ...
                      placeholder={t("literaturePage.gapMemoPlaceholder")}
                    />
                  </div>
                </div>
              ) : (
                <div className="flex items-center justify-center min-h-[300px] text-white/15 text-[16px]">
                  {t("literaturePage.searchAndRunGapHint")}
                </div>
              )}
            </div>
          )}
              ...
              <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04] min-h-[400px]">
                <div className="overflow-x-auto">
                  <table className="w-full text-[15px]">
                    <thead>
                      <tr className="text-white/30 border-b border-white/[0.06]">
                        <th className="text-left py-2 px-3">{t("literaturePage.authors")}</th>
                        <th className="text-left py-2 px-3">{t("literaturePage.year")}</th>
                        <th className="text-left py-2 px-3">{t("literaturePage.titleCol")}</th>
                        <th className="text-left py-2 px-3">{t("literaturePage.keywords")}</th>
                        <th className="text-left py-2 px-3">{t("literaturePage.memo")}</th>
                      </tr>
                    </thead>
                    <tbody className="text-white/50">
                        ...
                        <tr key={ref.id} className="border-b border-white/[0.03]">
                          ...
                          <td className="py-2 px-3">
                            <input
                              ...
                              placeholder={t("literaturePage.memoPlaceholder")}
                            />
                          </td>
                        </tr>
                      )) : (
                        <tr>
                          <td colSpan={5} className="py-8 text-center text-white/15">{t("literaturePage.addPapersToMatrixHint")}</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
