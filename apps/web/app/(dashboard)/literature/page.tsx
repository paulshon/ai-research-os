    ...
    <div className="flex flex-col font-nanum-gothic">
      ...
      <div className="flex items-center gap-1 px-4 py-2 bg-[#13161e] border-b border-white/[0.04] overflow-x-auto scrollbar-none">
        {LIT_TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`px-3 py-1.5 rounded-lg text-[12px] whitespace-nowrap transition-all flex items-center gap-1.5 ${
      ...
      <div className="overflow-y-auto p-4 md:p-6 lg:p-8">
        <div className="max-w-6xl mx-auto">
          {tab === "search" && (
            <div>
              <h2 className="text-[18px] font-bold font-nanum-myeongjo mb-4">논문 검색 & 탐색</h2>
              <div className="flex gap-3 mb-4">
                <input
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && handleSearch()}
                  className="flex-1 px-4 py-3 bg-[#13161e] border border-white/[0.06] rounded-xl text-[13px] text-white placeholder:text-white/20 focus:outline-none focus:border-[#3ecfb2]"
                  placeholder="키워드, DOI, 논문 제목으로 검색..."
                />
                <button
                  onClick={handleSearch}
                  disabled={searchLoading}
                  className="px-5 py-3 bg-[#3ecfb2]/15 border border-[#3ecfb2]/30 text-[#3ecfb2] rounded-xl text-[13px] font-medium hover:bg-[#3ecfb2]/25 transition-all disabled:opacity-40"
                >
              ...
              <div className="flex items-center gap-2 mb-5">
                ...
                <div className="ml-auto">
                  <span className="text-[10px] text-white/20">수집된 논문: {references.length}편</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
                ...
                <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04] min-h-[300px]">
                  <p className="text-[13px] text-white/25 mb-3">검색 결과 / 참고문헌 리스트</p>
                  {references.length > 0 ? (
                        ...
                        <tbody className="text-white/50">
                          {references.map((ref) => (
                            <tr key={ref.id} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                              ...
                              <td className="py-2 px-2">
                                {ref.url && ref.url !== "#" ? (
                                  <a href={ref.url} target="_blank" rel="noopener" className="text-[#3ecfb2] hover:underline text-[10px]">
                                    원문 →
                                  </a>
                                ) : "-"}
                              </td>
                              <td className="py-2 px-2">
                                <button onClick={() => removeRef(ref.id)} className="text-white/20 hover:text-[#ff7066] text-[10px]"><Icon name="✕" className="inline-flex align-[-0.125em] mr-1" size={15} /></button>
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    <div className="flex items-center justify-center h-[200px] text-white/15 text-[13px]">
                      키워드를 입력하여 논문을 검색하세요
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04]">
                    <p className="text-[13px] font-semibold text-[#3ecfb2] mb-3"><Icon name="🤖" className="inline-flex align-[-0.125em] mr-1" size={15} />AI Literature Copilot</p>
                    <p className="text-[12px] text-white/35 mb-3">관련 논문 추천, 자동 요약, 연구 갭 분석</p>
                    <Link href="/literature-review" className="text-[11px] text-[#3ecfb2] hover:underline block mb-2">→ 상세 문헌연구 엔진</Link>
                    <button
                      onClick={runGapAnalysis}
                      disabled={aiLoading || references.length === 0}
                      className="w-full py-2 rounded-lg bg-[#ec4899]/15 text-[#ec4899] text-[12px] font-medium disabled:opacity-30 transition-all"
                    >
          ...
          {tab === "gap" && (
            <div>
              <h2 className="text-[18px] font-bold font-nanum-myeongjo mb-4"><Icon name="🔬" className="inline-flex align-[-0.125em] mr-1" size={15} />연구갭 분석 결과</h2>
              {gapResult ? (
                <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04]">
                  <div className="text-[13px] text-white/70 leading-[1.8] whitespace-pre-wrap mb-4">{gapResult}</div>
                  <div className="border-t border-white/[0.06] pt-4">
                    <p className="text-[12px] text-white/30 mb-2">메모 추가</p>
                    <textarea
                      value={userNotes}
                      onChange={(e) => setUserNotes(e.target.value)}
                      className="w-full px-3 py-2 bg-[#1a1e2a] border border-white/[0.06] rounded-lg text-[12px] text-white focus:outline-none focus:border-[#3ecfb2] min-h-[100px] resize-y"
                      placeholder="연구갭 분석에 대한 메모를 추가하세요..."
                    />
                  </div>
                </div>
              ) : (
                <div className="flex items-center justify-center min-h-[300px] text-white/15 text-[13px]">
          ...
          {tab === "reading" && (
            <div>
              <h2 className="text-[18px] font-bold font-nanum-myeongjo mb-4"><Icon name="📖" className="inline-flex align-[-0.125em] mr-1" size={15} />읽기 공간</h2>
              <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04] min-h-[400px]">
                <textarea
                  value={userNotes}
                  onChange={(e) => setUserNotes(e.target.value)}
                  className="w-full min-h-[350px] bg-transparent text-[13px] text-white/70 leading-[1.8] resize-none focus:outline-none"
          ...
          {tab === "matrix" && (
            <div>
              <h2 className="text-[18px] font-bold font-nanum-myeongjo mb-4">문헌 비교 매트릭스</h2>
              <div className="p-5 rounded-[16px] bg-[#13161e] border border-white/[0.04] min-h-[400px]">
                <div className="overflow-x-auto">
                        ...
                        <tr key={ref.id} className="border-b border-white/[0.03]">
                          ...
                          <td className="py-2 px-3">
                            <input
                              ...
                              className="w-full bg-transparent text-[11px] text-white/50 focus:outline-none focus:text-white/70 border-b border-transparent focus:border-white/10"
                              placeholder="메모..."
                            />
                          </td>
                        </tr>
          ...
          {tab === "collections" && (
            <div className="flex items-center justify-center min-h-[400px]">
              <div className="text-center">
                <p className="text-[32px] mb-3 flex justify-center"><Icon name="folder" size={30} /></p>
                <p className="text-[15px] text-white/40 font-medium">컬렉션</p>
                <p className="text-[12px] text-white/20 mt-2">참고문헌을 주제별 컬렉션으로 묶는 기능은 준비 중입니다</p>
              </div>
            </div>
          )}
        </div>
