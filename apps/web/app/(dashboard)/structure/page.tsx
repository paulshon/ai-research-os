      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        ...
        <div className="flex items-center justify-between gap-2 mb-4 text-[15px] text-white/30">
          <div className="flex items-center gap-2 min-w-0">
            <span className="truncate">{chapters[0]?.title?.split(" (")[0] ?? "서론"} ({chapters[0]?.title?.split(" (")[1]?.replace(")", "") ?? "Introduction"})</span>
          ...
          <div className="flex items-center gap-3 text-[14px] flex-shrink-0">
            ...
            <Link href="/editor/new" className="text-[#a78bfa] hover:underline inline-flex items-center"><Icon name="✦" className="inline-flex align-[-0.125em] mr-1" size={15} />상세 에디터</Link>
          </div>
        </div>
        ...
        <div className="mb-5">
          <h1 className="text-[23px] font-bold font-nanum-myeongjo mb-1 flex items-center gap-2">
            {isAdminEntry ? (
              ...
              <span className="opacity-30 cursor-not-allowed select-none" title="관리자 전용" aria-disabled="true"><Icon name="⬡" className="inline-flex align-[-0.125em] mr-1" size={15} /></span>
            )}
        ...
        <div className="flex-shrink-0 border-b border-white/[0.04] bg-[#0d0f14]">
          <div className="px-4 pt-3 pb-2">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-[14px] text-white/40">{copilotEnabled ? "켜짐" : "꺼짐"}</span>
              <div className="flex-1" />
              <span className="text-[13px] text-white/20">1 채팅</span>
              <span className="text-[13px] text-white/20">0 검증</span>
              <span className="text-[13px] text-white/20">1 인용</span>
            </div>
          </div>

          {/* Copilot context info */}
          <div className="px-4 pb-3">
            <p className="text-[14px] text-white/25 mt-1">현재 문서 컨텍스트를 기반으로 AI가 도움을 드립니다.</p>
          </div>
