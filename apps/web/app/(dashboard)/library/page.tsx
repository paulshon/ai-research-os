    ...
    <div className="flex flex-col md:flex-row font-nanum-gothic">
      ...
      <div className="md:hidden flex items-center gap-2 px-3 py-2 border-b border-white/[0.04] bg-[#0d0f14] sticky top-0 z-[40]">
        ...
        {selectedChapter !== null && chapters[selectedChapter] && (
          <span className="text-[10px] text-white/30 truncate">{chapters[selectedChapter].title}</span>
        )}
      </div>
      ...
      <div className={`${mobileSidebar ? "fixed inset-y-0 left-0 z-[7900] flex animate-slide-in-left" : "hidden"} md:relative md:z-auto md:flex w-80 max-w-[85vw] border-r border-white/[0.04] bg-[#0d0f14] flex-col overflow-y-auto flex-shrink-0`}>
        ...
        <div className="p-4 border-b border-white/[0.04]">
          <h2 className="text-[14px] font-semibold"><Icon name="📚" className="inline-flex align-[-0.125em] mr-1" size={15} />{t("library.title")}</h2>
          <p className="text-[11px] text-white/25 mt-1">{t("library.desc")}</p>
        </div>
        <div className="p-3">
          <select value={thesisType} onChange={e => { setThesisType(e.target.value); setSelectedChapter(null); setPatterns(""); }}
            className="w-full px-3 py-2 rounded-lg bg-[#1a1e2a] border border-white/[0.06] text-[12px] text-white/60 mb-3">
            {categories.map(cat => (
          ...
          <div className="space-y-1">
            {chapters.map((ch, i) => (
              <button key={i} onClick={() => loadPatterns(i)}
                ...
                <span className="text-[10px] font-['JetBrains_Mono',monospace] mr-2" style={{ color: ch.color }}>{ch.num}</span>
                {ch.title}
              </button>
            ))}
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        {patterns ? (
          <div className="max-w-3xl">
            ...
            <div className="text-[13px] text-white/70 leading-[1.8] whitespace-pre-wrap">{patterns}</div>
          </div>
        ) : loading ? (
          ...
          <div className="text-center py-20 text-white/15">
            <p className="text-[24px] mb-2 flex justify-center"><Icon name="library" size={28} /></p>
            <p className="text-[14px]">{t("library.emptyTitle")}</p>
            <button
              onClick={() => setMobileSidebar(true)}
              className="md:hidden mt-4 px-4 py-2 rounded-lg bg-[#6c8cff]/15 border border-[#6c8cff]/30 text-[#6c8cff] text-[12px] font-medium inline-flex items-center gap-1.5"
            >
