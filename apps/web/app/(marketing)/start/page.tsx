...
const TRACK_BULLETS: Record<"rdos" | "aros", string[]> = {
  rdos: [
    "연구 기초 · 논문 구조 학습",
    "연구설계/연구방법론 기초",
    "논문 읽기 훈련 · APA 학습",
    ...
    <div className="min-h-screen bg-[#070708] text-[#f4f4f5] font-nanum-gothic antialiased">
      ...
      <section className="pt-[140px] pb-28 px-6">
        <div className="max-w-[1080px] mx-auto text-center mb-14">
          <p className="text-[12px] uppercase tracking-[0.22em] text-white/35 mb-6 font-mono">choose your track</p>
          ...
          <p className="text-[15px] text-white/50 max-w-[560px] mx-auto leading-relaxed">
            가입 단계에서 트랙을 선택하면, 인증·승인 후 각 플랫폼으로 분기됩니다.
            연구 역량을 기르는 <span className="text-[#3ecfb2]">RDOS</span> 와
            실제 연구를 수행하는 <span className="text-[#ff7a00]">AI-Research-OS</span>.
          </p>
        </div>

        <div className="max-w-[920px] mx-auto grid md:grid-cols-2 gap-6">
          {[rdos, aros].map((tr) => {
            const bullets = TRACK_BULLETS[tr.id];
            return (
              <div
                key={tr.id}
              ...
              >
                ...
                <h2 className="text-[21px] font-bold tracking-tight mb-1">{tr.title}</h2>
                ...
                <p className="text-[13.5px] text-white/55 leading-[1.7] mb-5"><span className="text-white/40">목적</span> · {tr.purpose}</p>
        ...
        <div className="max-w-[920px] mx-auto mt-10 p-5 rounded-[16px] bg-[#0c0c0e] border border-white/[0.08]">
          <p className="text-[11px] text-white/35 font-mono mb-3">user flow</p>
          <div className="flex flex-wrap items-center gap-2 text-[11px]">
            {["랜딩", "→", "플랜 선택", "→", "회원가입", "→", "인증", "→", "관리자 승인", "→", "RDOS / AI-Research-OS"].map((s, i) => (
