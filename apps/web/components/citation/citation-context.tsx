...
interface CitationContextValue {
  ...
  updateRef: (id: string, patch: Partial<RefEntry>) => void;
...
export function CitationProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateRef = useCallback((id: string, patch: Partial<RefEntry>) => {
    setRefDB((prev) => {
      const next = prev.map((r) => (r.id === id ? { ...r, ...patch, id: r.id } : r));
      saveRefDB(next);
      return next;
    });
  }, []);
