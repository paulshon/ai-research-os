...
import { localizeGrowthLevel } from "@/lib/rdos/rdos-display-i18n";
...
export default function RdosRoadmapView({ levels, xp, currentCode }: { levels: Level[]; xp: number; currentCode: string }) {
  ...
  return (
    <div>
      <Link href="/rdos" className="text-[12px] text-white/40 hover:text-white/80 transition">{t("rdos.roadmap.backToDashboard")}</Link>
      <h2 className="text-[17px] font-extrabold tracking-tight mt-4 mb-1">{t("rdos.roadmap.title")}</h2>
      <p className="text-[14px] text-white/55 mb-8">{t("rdos.roadmap.subtitle").replace("{xp}", String(xp))}</p>
              ...
              <div key={lv.code} className="relative flex items-center gap-4">
                ...
                <div className={`flex-1 p-4 rounded-[14px] border ${current ? "bg-[#1a1109] border-[#ff7a00]/50" : reached ? "bg-[#161009] border-[#ff7a00]/25" : "bg-[#10131a] border-white/[0.07]"}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-[15px] font-bold">{lv.code} · {localizeGrowthLevel(lv, locale)} {current && <span className="text-[11px] text-[#ff7a00] ml-1">{t("rdos.roadmap.current")}</span>}</span>
                    <span className="text-[11px] font-mono text-white/40">{lv.minXp} XP</span>
                  </div>
                  {locale === "ko" && <p className="text-[12px] text-white/45 font-mono mt-0.5">{lv.en}</p>}
                </div>
              </div>
            );
          })}
