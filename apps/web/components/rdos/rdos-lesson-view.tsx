...
export default function RdosLessonView({ content: rawContent, initialDone }: { content: LessonContent; initialDone: number }) {
  ...
  /* s-renew-13: 사이드바 학습 트리에서 ?lesson=<id> 로 들어오면 해당 레슨을 바로 연다. */
  const searchParams = useSearchParams();
  useEffect(() => {
    const lid = searchParams?.get("lesson");
    if (!lid) return;
    const idx = content.lessons.findIndex((l) => l.id === lid);
    if (idx >= 0) setOpenIdx(idx);
  }, [searchParams, content.lessons]);
    ...
    <div>
      ...
      <div className="flex items-start gap-3 mt-4 mb-5">
        ...
        <div className="flex-1">
          <h2 className="text-[17px] font-extrabold tracking-tight">{content.label}</h2>
