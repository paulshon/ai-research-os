    ...
    <div className="mb-5">
      ...
      {revealed && (
        <div className={`mt-1 text-[12px] leading-relaxed rounded-[9px] px-3 py-2 ${selected === q.answer ? "bg-[#3ecfb2]/10 text-[#7fe6cf]" : "bg-[#ff7066]/10 text-[#ffb0aa]"}`}>
          {selected === q.answer ? "✓ 정답입니다. " : "✗ 오답입니다. "}{q.explanation}
        </div>
      )}
    </div>
  );
      ...
      <div className="w-full max-w-[680px] max-h-[88vh] flex flex-col rounded-[16px] bg-[#0d0f14] border border-white/[0.1] overflow-hidden">
        ...
        <div className="flex gap-2 px-6 pt-4">
          {(["content", "quiz"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              ...
              style={{ background: tab === t ? accent + "22" : "#161a22", color: tab === t ? accent : "#8a92a6" }}>
              {t === "content" ? "📖 학습 내용" : "📝 퀴즈"}
            </button>
          ))}
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {tab === "content" ? (
            ...
            <>
              <div className="text-[13px] font-bold text-white/80 mb-3">이해도 퀴즈</div>
              {lesson.quiz.map((q, i) => (
        ...
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-white/[0.07]">
          ...
          {alreadyDone ? (
            <span className="text-[12px] text-[#3ecfb2]">✓ 완료된 레슨</span>
          ) : (
            <button onClick={() => canComplete && onComplete()} disabled={!canComplete}
              ...
              {tab !== "quiz" ? "퀴즈를 풀어야 완료됩니다" : allAnswered ? "✓ 레슨 완료" : "퀴즈를 모두 풀어주세요"}
            </button>
          )}
        </div>
      </div>
...
export default function RdosLessonView({ content, initialDone }: { content: LessonContent; initialDone: number }) {
  ...
  function persist(next: number) {
    startTransition(async () => {
      try {
        ...
        setMsg(json.persisted ? "진행이 저장되었습니다 (대시보드 반영)" : "진행 기록(로컬) — 서버 미연결");
        router.refresh();
      } catch {
        setMsg("저장 실패 — 네트워크 확인");
      }
    });
  }
    ...
    <div>
      ...
      <div className="flex items-start gap-3 mt-4 mb-5">
        ...
        <div className="flex-1">
          ...
          <p className="text-[12px] text-white/40 font-mono">제공: {content.provides} · engine: {content.engine}</p>
        </div>
        <div className="text-right flex-shrink-0">
          <p className="text-[11px] text-white/40 font-mono">미션 진행</p>
      ...
      <div className="grid sm:grid-cols-3 gap-3 mb-9">
        {content.objectives.map((o, i) => (
          <div key={i} className="p-4 rounded-[13px] bg-[#10131a] border border-white/[0.07]">
            <span className="text-[11px] font-mono" style={{ color: ACCENT }}>목표 {i + 1}</span>
            <p className="text-[12.5px] text-white/70 mt-1 leading-relaxed">{o}</p>
          </div>
        ))}
      </div>

      <h2 className="text-[14px] font-bold text-white/80 mb-3">학습 모듈 (클릭하여 학습)</h2>
          ...
          return (
            <button key={i} disabled={!clickable} onClick={() => clickable && setOpenIdx(i)}
              ...
              <div className="flex-1">
                ...
                <p className="text-[12px] text-white/45">{lesson.subtitle} · 퀴즈 {lesson.quiz.length}문항</p>
              </div>
              <span className="text-[11px] font-mono flex-shrink-0" style={{ color: st === "done" ? "#3ecfb2" : st === "active" ? ACCENT : "rgba(255,255,255,0.3)" }}>{st === "done" ? "완료" : st === "active" ? "학습하기 →" : "잠금"}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="p-5 rounded-[14px] bg-[#0e1118] border border-white/[0.06]">
        <p className="text-[12px] text-white/45 mb-2">이 미션 완료 시 (커널 보상)</p>
        <div className="flex flex-wrap gap-2">
          <span className="px-2.5 py-1 rounded-full text-[11px] bg-[#ff7a00]/12 text-[#ff7a00] border border-[#ff7a00]/20">XP +15/레슨</span>
        ...
        <p className="text-[11px] text-white/35 mt-3">연결 엔진: <span style={{ color: ACCENT }}>{content.engine}</span> · 진행은 Supabase에 저장되어 대시보드(커널 도출)에 반영됩니다.</p>
      </div>
