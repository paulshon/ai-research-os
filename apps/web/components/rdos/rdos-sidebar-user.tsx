...
function formatSavedAt(iso: string | null): string {
  if (!iso) return "아직 저장한 기록이 없어요";
  const d = new Date(iso);
  if (isNaN(d.getTime())) return "저장됨";
  const now = new Date();
  const sameDay = d.toDateString() === now.toDateString();
  const hh = String(d.getHours()).padStart(2, "0");
  const mm = String(d.getMinutes()).padStart(2, "0");
  if (sameDay) return `오늘 ${hh}:${mm} 저장됨`;
  const mo = String(d.getMonth() + 1).padStart(2, "0");
  const dd = String(d.getDate()).padStart(2, "0");
  return `${mo}.${dd} ${hh}:${mm} 저장됨`;
}
...
export default function RdosSidebarUser({ compact = false }: { compact?: boolean }) {
  ...
  const displayName = user?.fullName ?? user?.firstName ?? "연구준비자";
  ...
  const saveLabel =
    state === "saving" ? "저장 중…" : state === "saved" ? "저장됨" : state === "error" ? "다시 시도" : "저장";
    ...
    <div className="mt-auto border-t border-white/[0.06] px-3 py-3 space-y-2.5 bg-[#0a0c10]">
        ...
        {!compact && (
          <div className="flex-1 min-w-0">
            <p className="text-[13.5px] font-semibold truncate text-[#e8eaf0] leading-tight">
              {isLoaded || !hasClerk ? displayName : "불러오는 중…"}
            </p>
            {email && (
              <p className="text-[11.5px] truncate text-[#626880] leading-tight mt-0.5">{email}</p>
            )}
            <p className="text-[10.5px] text-[#4b5169] leading-tight mt-1">{formatSavedAt(savedAt)}</p>
          </div>
        )}
      </div>
      ...
      {!hasClerk && !compact && (
        <p className="text-[10px] text-white/25 leading-snug px-1">
          로그인(Clerk) 설정 시 사용자 정보와 서버 저장이 자동 연동됩니다.
        </p>
      )}
    </div>
  );
