...
export default function RdosApprovalList({ initial }: { initial: Enrollment[] }) {
  ...
  if (rows.length === 0) return <p className="text-[13px] text-white/40">가입 신청이 없습니다. (Supabase 미연결 시 비어 있습니다.)</p>;

  return (
    <div className="space-y-2">
      {rows.map((e) => {
        const name = (e.profile?.name as string) ?? "(이름 없음)";
          ...
          <div key={e.user_id} className="flex items-center gap-4 p-4 rounded-[12px] bg-[#13161e] border border-white/[0.07]">
            ...
            <div className="flex gap-2">
              <button onClick={() => decide(e.user_id, true)} disabled={busy === e.user_id || e.status === "approved"}
                className="px-3 py-1.5 rounded-[8px] text-[12px] font-semibold bg-[#3ecfb2] text-[#070708] disabled:opacity-40">승인</button>
              <button onClick={() => decide(e.user_id, false)} disabled={busy === e.user_id}
                className="px-3 py-1.5 rounded-[8px] text-[12px] font-medium border border-white/[0.12] text-white/65 disabled:opacity-40">거절</button>
            </div>
          </div>
        );
      })}
