...
function buildToolbar(t: (k: string) => string): ToolbarAction[] {
  return [
    { kind: "format", block: "h1", icon: "note", title: t("editorToolbar.h1"), label: "H1" },
    { kind: "format", block: "h2", icon: "note", title: t("editorToolbar.h2"), label: "H2" },
    { kind: "format", block: "h3", icon: "note", title: t("editorToolbar.h3"), label: "H3" },
    { kind: "format", block: "p", icon: "file", title: t("editorToolbar.body"), label: t("editorToolbar.body") },
    { kind: "cmd", command: "bold", icon: "pencil", title: t("editorToolbar.bold") },
    { kind: "cmd", command: "italic", icon: "pencil", title: t("editorToolbar.italic") },
    { kind: "cmd", command: "underline", icon: "pencil", title: t("editorToolbar.underline") },
    { kind: "cmd", command: "insertUnorderedList", icon: "files", title: t("editorToolbar.bulletList") },
    { kind: "cmd", command: "insertOrderedList", icon: "files", title: t("editorToolbar.numberedList") },
    { kind: "format", block: "blockquote", icon: "files", title: t("editorToolbar.quote"), label: "“ ”" },
  ];
}
...
const RichTextEditor = forwardRef<RichTextEditorHandle, RichTextEditorProps>(
  function RichTextEditor({ value, onChange, placeholder, minHeight = 500 }, ref) {
    ...
    const TOOLBAR = useMemo(() => buildToolbar(t), [t]);
    ...
    return (
      <div className="rich-text-editor">
        ...
        <div
          ...
          data-placeholder={placeholder || t("editorToolbar.placeholder")}
        ...
        {selectedEl && rect && (
          ...
          >
            <span className="px-1.5 text-white/35">{isImg ? t("common.figure") : t("common.table")}</span>
            <span className="w-px h-4 bg-white/10 mx-0.5" />
            <button onClick={() => applyWidth(40)} className="px-2 py-1 rounded-md text-white/55 hover:text-white hover:bg-white/[0.07]" title={t("editorToolbar.mediaSmall")}>{t("editorToolbar.mediaSmall")}</button>
            <button onClick={() => applyWidth(70)} className="px-2 py-1 rounded-md text-white/55 hover:text-white hover:bg-white/[0.07]" title={t("editorToolbar.mediaMedium")}>{t("editorToolbar.mediaMedium")}</button>
            <button onClick={() => applyWidth(100)} className="px-2 py-1 rounded-md text-white/55 hover:text-white hover:bg-white/[0.07]" title={t("editorToolbar.mediaLarge")}>{t("editorToolbar.mediaLarge")}</button>
            <span className="w-px h-4 bg-white/10 mx-0.5" />
            <button onClick={() => applyAlign("left")} className="px-1.5 py-1 rounded-md text-white/55 hover:text-white hover:bg-white/[0.07]" title={t("editorToolbar.alignLeft")}><Icon name="alignLeft" size={14} /></button>
            <button onClick={() => applyAlign("center")} className="px-1.5 py-1 rounded-md text-white/55 hover:text-white hover:bg-white/[0.07]" title={t("editorToolbar.alignCenter")}><Icon name="alignCenter" size={14} /></button>
            <button onClick={() => applyAlign("right")} className="px-1.5 py-1 rounded-md text-white/55 hover:text-white hover:bg-white/[0.07]" title={t("editorToolbar.alignRight")}><Icon name="alignRight" size={14} /></button>
            <span className="w-px h-4 bg-white/10 mx-0.5" />
            <button onClick={toggleInline} className="px-2 py-1 rounded-md text-white/55 hover:text-white hover:bg-white/[0.07]" title={t("editorToolbar.inlineBlock")}>{t("editorToolbar.inlineBlockLabel")}</button>
            <button onClick={removeEl} className="px-1.5 py-1 rounded-md text-[#ff7066]/80 hover:text-[#ff7066] hover:bg-[#ff7066]/10" title={t("editorToolbar.remove")}><Icon name="trash" size={14} /></button>
          </div>
        )}
        ...
        {isImg && rect && (
          <div
            ...
            title={t("editorToolbar.resizeHint")}
          />
        )}
