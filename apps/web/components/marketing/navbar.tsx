...
export default function MarketingNavbar() {
  ...
  const NAV_LINKS = [
    { href: "/rdos-signup", label: t("startPage.navRdos") },
    { href: "/researcher-signup", label: t("startPage.navAros") },
    ...
    { href: "/contact", label: t("landing2.navContact") },
  ];
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-2.5 group flex-shrink-0 whitespace-nowrap">
          ...
          <span className="font-nanum-myeongjo text-[17px] font-semibold tracking-tight whitespace-nowrap">
            AI Research <span className="text-[#e8b84b]">OS</span>
          </span>
        </Link>

        <div className="hidden xl:flex items-center gap-5 min-w-0 flex-nowrap">
          {NAV_LINKS.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="text-[13px] text-white/40 hover:text-white/80 transition-colors duration-200 whitespace-nowrap"
            >
              {l.label}
            </Link>
          ))}
        ...
        <button
          ...
          className="xl:hidden text-white/50 hover:text-white p-2 text-xl flex-shrink-0"
        >
      ...
      {mobileOpen && (
        <div className="xl:hidden bg-[#13161e]/98 backdrop-blur-xl border-t border-white/[0.04] px-6 py-5 space-y-1">
          {NAV_LINKS.map((l) => (
            <Link
              key={l.href}
              href={l.href}
