...
/* ════════════════════════════════════════════════════════════
   s-renew-12: Studium R — 인트로 스플래시 (임팩트 제거 버전)
   시퀀스:
      ...
      <div className="relative z-10 flex flex-col items-center px-6">
        <div className="logo-container relative rounded-[26px] opacity-0" style={{ willChange: "transform, opacity, filter, box-shadow" }}>
          <div
            ...
            style={{ background: "radial-gradient(circle, #E0A73Eaa, transparent 70%)" }}
          />
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src="/brand/appicon_brass.svg"
            alt="Studium R"
            className="relative w-24 h-24 md:w-32 md:h-32 object-contain drop-shadow-[0_0_30px_rgba(224,167,62,0.55)]"
            draggable={false}
          />
        </div>

        <div className="text-group mt-6 text-center opacity-0">
          {/* s-renew-13: 글자 크기는 기존 그대로, 색만 Studium R 브랜드 신색상 적용 */}
          <h1 className="text-3xl md:text-5xl font-semibold tracking-tight font-nanum-myeongjo leading-none">
            <span className="text-[#F4F1EA]">Studium</span>
            <span className="text-[#E0A73E] ml-[0.15em]">R</span>
          </h1>
          {/* 태그라인: 기존 크기·기존 색(회색) 유지, 문구만 교체 */}
          <p className="mt-3 text-[11px] md:text-[13px] tracking-[0.22em] text-white/35 font-mono">
            AI Research Operating System for Scholars
          </p>
        </div>
        ...
        <div
          ...
          style={{
            transform: "scaleX(0)",
            background:
              "linear-gradient(90deg, transparent 0%, rgba(224,167,62,0.55) 16%, rgba(224,167,62,0.95) 55%, rgba(244,241,234,0.9) 100%)",
            boxShadow: "0 0 14px rgba(224,167,62,0.45)",
          }}
        />
      </div>
    </motion.div>
